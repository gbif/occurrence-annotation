...
try {
    ...
    Write-Host "   ✅ basisOfRecord: $($createdRule3.basisOfRecord ?? 'null')" -ForegroundColor Green
    ...
    foreach ($rule in $finalRules) {
        Write-Host "     - Rule $($rule.id): taxonKey=$($rule.taxonKey), basisOfRecord=$($rule.basisOfRecord ?? 'null'), annotation=$($rule.annotation)" -ForegroundColor Gray
    }
