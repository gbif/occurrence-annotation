...
import { Search, ExternalLink, Loader2, MapPin, Calendar, User, Database, Eye } from 'lucide-react';
...
export function InvestigateArea({ 
  ...
  const [searchRadius] = useState(2000); // Default 2km radius
  ...
  const investigateArea = useCallback(async (lat: number, lng: number) => {
    ...
    try {
      // Create a WKT polygon approximating a circle around the clicked point
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        const pointLng = lng + radiusInDegrees * Math.sin(angle) / Math.cos(lat * Math.PI / 180); // Adjust for latitude
      ...
      const wktGeometry = `POLYGON((${circlePoints.join(',')}))`;
      
      console.log('🔍 InvestigateArea: Generated WKT geometry:', wktGeometry);
      
      // Search for occurrences within the circular area
      const apiUrl = `https://api.gbif.org/v1/occurrence/search?` +
        `taxonKey=${selectedSpecies.key}&` +
        `hasCoordinate=true&` +
        `geometry=${encodeURIComponent(wktGeometry)}&` +
        `limit=50`;
