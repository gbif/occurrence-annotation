...
export function AnnotationRules({ 
  ...
  function getBasicAuthHeader() {
    const gbifUser = localStorage.getItem('gbifUser') || '';
    const gbifPwd = localStorage.getItem('gbifPwd') || '';
    if (!gbifUser || !gbifPwd) return null;
    return 'Basic ' + btoa(gbifUser + ':' + gbifPwd);
  }
  ...
  const handleVote = async (ruleId: number, action: 'support' | 'contest') => {
    ...
    try {
      ...
      const endpoint = action === 'support' 
        ? `http://localhost:8080/v1/occurrence/annotation/rule/${ruleId}/support`
        : `http://localhost:8080/v1/occurrence/annotation/rule/${ruleId}/contest`;
  ...
  const handleRemoveVote = async (ruleId: number, action: 'support' | 'contest') => {
    ...
    try {
      ...
      const endpoint = action === 'support' 
        ? `http://localhost:8080/v1/occurrence/annotation/rule/${ruleId}/removeSupport`
        : `http://localhost:8080/v1/occurrence/annotation/rule/${ruleId}/removeContest`;
