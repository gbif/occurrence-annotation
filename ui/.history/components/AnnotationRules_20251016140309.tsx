...
import { AlertCircle, CheckCircle, XCircle, Trash2, ChevronDown, ChevronUp, MessageSquare, Loader2, Pencil, ThumbsUp, ThumbsDown } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchRules = async (page: number = 0) => {
      ...
      try {
        ...
        // Fetch rules for all taxon keys with pagination
        const allRulesPromises = taxonKeys.map(async ({ key, level }) => {
          const response = await fetch(
            `https://api.gbif.org/v1/occurrence/experimental/annotation/rule?taxonKey=${key}&limit=${pageSize}&offset=${offset}`
          );
          ...
          const rules: AnnotationRule[] = Array.isArray(data) ? data : data.results || [];
          ...
          return { rules: enrichedRules, count };
        });
        
        const allRulesResults = await Promise.all(allRulesPromises);
        const allRules = allRulesResults.flatMap(result => result.rules);
        const totalRulesCount = allRulesResults.reduce((sum, result) => sum + result.count, 0);
      ...
      {totalCount > pageSize && (
        <div className="flex items-center justify-between gap-2 pt-2 border-t">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>•</span>
            <span>{Math.min((currentPage + 1) * pageSize, totalCount)} of {totalCount} rules</span>
          </div>
