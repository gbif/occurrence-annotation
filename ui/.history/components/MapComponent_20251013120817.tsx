...
export function MapComponent({
  ...
  const handleMapClick = ({ latLng }: { latLng: [number, number] }) => {
    ...
    const [screenX, screenY] = latLngToStablePixel(lat, lng);
    
    console.log('🖱️ Map Click Debug:', {
      coordinates: { lat, lng },
      screenPosition: { x: screenX.toFixed(1), y: screenY.toFixed(1) },
      withinWebMercatorLimits: Math.abs(lat) <= WEB_MERCATOR_MAX_LAT,
        ...
        >
          ...
          >
            ...
            {(() => {
              ...
              // Add specific test markers for Web Mercator limits
              const testPoints = [
