...
import { Trash2, Square, Check, X, Edit2, Search, Plus, Minus } from 'lucide-react';
...
export function MapComponent({
  ...
  const investigateArea = async (lat: number, lng: number) => {
    ...
    // Implementation will be added here - for now just log
    toast.info(`Investigating ${investigateRadius/1000}km area for ${selectedSpecies.scientificName}`);
  };
              ...
              {isInvestigateMode && (
                <div className="flex items-center gap-1 bg-gray-50 rounded px-2 py-1">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    className="h-6 w-6"
                    onClick={() => setInvestigateRadius(Math.min(20000, investigateRadius + 1000))}
