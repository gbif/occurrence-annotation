...
export function MapComponent({
  ...
  // Refs to track stable map state for polygon rendering
  const stableMapRef = useRef<{ center: [number, number]; zoom: number }>({ center, zoom });
  const isMapDragging = useRef(false);
  const dragStartRef = useRef<{ center: [number, number]; zoom: number } | null>(null);
  ...
  // Update stable map reference only when map is not being dragged
  useEffect(() => {
    if (!isMapDragging.current) {
      stableMapRef.current = { center, zoom };
      console.log('📍 Stable map ref updated:', { center, zoom });
    } else {
      console.log('📍 Stable map ref FROZEN during drag - current:', stableMapRef.current, 'new:', { center, zoom });
    }
  }, [center, zoom]);
  ...
  const latLngToStablePixel = useCallback((lat: number, lng: number): [number, number] => {
    // Use drag start coordinates if currently dragging, otherwise use stable ref
    const mapState = isMapDragging.current && dragStartRef.current 
    ...
    if (isMapDragging.current) {
      console.log('🎯 Using FROZEN coordinates for polygon:', { 
        frozenCenter: mapState.center, 
        frozenZoom: mapState.zoom,
    ...
    return [x, y];
  }, [mapSize.width, mapSize.height, center, zoom]); // Include center/zoom so we get updates when not dragging
  ...
  return (
    <div 
      ...
      onMouseDown={() => {
        // Capture the exact map state when drag starts
        dragStartRef.current = { center, zoom };
        isMapDragging.current = true;
        console.log('🗺️ Map drag started - coordinates frozen at:', dragStartRef.current);
      }}
      onMouseLeave={() => {
        setMousePosition(null);
        handleMouseUp();
        // End map interaction when mouse leaves - but use a delay to ensure proper state
        setTimeout(() => {
          isMapDragging.current = false;
          dragStartRef.current = null;
          stableMapRef.current = { center, zoom };
          console.log('🗺️ Map interaction ended via mouse leave - stable ref updated');
        }, 150);
      }}
      onClick={(e) => {
      ...
      <Map
        ...
        onBoundsChanged={({ center, zoom: newZoom }) => {
          ...
          console.log('🗺️ BOUNDS CHANGED:', {
            ...
            oldZoom: zoom,
            newZoom: newZoom,
            isDragging: isMapDragging.current
          });
          ...
          // CRITICAL: Set dragging flag BEFORE state updates to prevent stable ref updates
          const wasDragging = isMapDragging.current;
          if (!wasDragging) {
            // Only update stable ref if this is not part of a drag operation
            // We detect this by checking if center/zoom changed significantly
            const centerDelta = Math.abs(center[0] - clampedCenter[0]) + Math.abs(center[1] - clampedCenter[1]);
            const zoomDelta = Math.abs(zoom - newZoom);
            
            if (centerDelta > 0.001 || zoomDelta > 0.01) {
              // This is likely a programmatic change or initial load, not a drag
              isMapDragging.current = false;
            }
          }
          ...
          // End map interaction when bounds change completes - longer delay for drag end detection
          setTimeout(() => {
            isMapDragging.current = false;
            stableMapRef.current = { center: clampedCenter, zoom: newZoom };
            console.log('🗺️ Map drag completed - stable ref updated');
          }, 100); // Increased delay to ensure drag has actually ended
        }}
        onClick={(event) => {
              ...
              <>
                {isCurrentInverted ? (
                  (() => {
                    ...
                    const holePixels = currentPolygon.map(([lat, lng]) => latLngToStablePixel(lat, lng));
                    pathStr += ` M ${holePixels.map(([x, y]) => `${x},${y}`).join(' L ')} Z`;
