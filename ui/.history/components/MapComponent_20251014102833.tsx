...
export function MapComponent({
  ...
  const latLngToStablePixel = useCallback((lat: number, lng: number): [number, number] => {
    // Always use stable ref values to prevent polygon drift during map drag
    const mapState = stableMapRef.current;
    ...
    const y = (worldY - centerWorldY) + mapSize.height / 2;
    return [x, y];
  }, [mapSize.width, mapSize.height]); // Only depend on map size, not center/zoom
