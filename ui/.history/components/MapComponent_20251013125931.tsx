...
export function MapComponent({
  ...
  useEffect(() => {
    ...
    if (stableCenter.current[0] === 20 && stableCenter.current[1] === 0 && stableZoom.current === 2) {
      ...
      console.log('🔧 Initializing stable coordinates:', { center, zoom, gbifTilesLoaded: gbifTiles.length > 0 });
      return;
    }
    ...
    if (stableZoom.current !== zoom) {
      stableZoom.current = zoom;
      stableCenter.current = center;
      console.log('🔧 Zoom changed - updating stable coordinates:', { 
        newCenter: center, 
        newZoom: zoom, 
        gbifTilesLoaded: gbifTiles.length > 0 
      });
    }
    // Update when transform clears (end of drag)
      ...
      stableCenter.current = center;
      console.log('🔧 Pan ended - updating stable center:', { 
        newCenter: center, 
        zoom, 
        gbifTilesLoaded: gbifTiles.length > 0 
      });
            ...
            {(() => {
              ...
              if (currentTileZ >= 1) {
                ...
                console.log('🧮 Tile boundary comparison:', {
                  ...
                  gbifTilesActive: gbifTiles.length > 0
                });
                
                // Warn if there's a significant coordinate mismatch
                ...
                if (northDiff > 1 || southDiff > 1) {
                  console.warn('⚠️ COORDINATE SYSTEM MISMATCH DETECTED:', {
                    northMismatch: northDiff.toFixed(2) + '°',
                    southMismatch: southDiff.toFixed(2) + '°',
                    possibleCause: 'GBIF tiles may be affecting coordinate reference'
                  });
                }
