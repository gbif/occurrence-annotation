...
import { Search, ExternalLink, Loader2, MapPin, Calendar, User, Database, Eye, X } from 'lucide-react';
...
interface InvestigateAreaProps {
  ...
  onAreaInvestigate?: (lat: number, lng: number, radius: number) => void;
}
...
export function InvestigateArea({ 
  ...
  const [searchRadius, setSearchRadius] = useState(1000); // Default 1km radius

  // Function to be called when user clicks on map in investigate mode
  const handleMapClick = async (lat: number, lng: number) => {
    if (!selectedSpecies || !isInvestigateMode) return;
    ...
    try {
      // Search for occurrences within radius of clicked point
      ...
      if (enrichedOccurrences.length === 0) {
        toast.info(`No occurrences found for ${selectedSpecies.scientificName} within ${searchRadius}m of this location`);
      } else {
  ...
  useEffect(() => {
    // The parent will call handleMapClick when a map click occurs in investigate mode
    // We don't need to do anything here as the parent handles the callback
  ...
  return (
    <>
      ...
      <Button
        ...
        size="icon"
        onClick={() => onToggleInvestigateMode(!isInvestigateMode)}
