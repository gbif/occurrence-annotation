...
import { Trash2, Square, Check, X, Pentagon, Edit2 } from 'lucide-react';
...
export function MapComponent({
  ...
  const handleOverlayClick = (e: React.MouseEvent) => {
    ...
    // For circle, we only need 2 points
    if (drawingMode === 'circle' && drawingPoints.length >= 1) {
      setDrawingPoints([...drawingPoints, [lat, lng]]);
      // Auto-finish after second point
      setTimeout(() => finishDrawing(), 10);
    } else {
      setDrawingPoints([...drawingPoints, [lat, lng]]);
    }
  };
          ...
          {!isDrawing ? (
            ...
            <>
              <div className="px-2 py-1 text-xs text-gray-600 whitespace-nowrap">
                ...
                {drawingMode === 'circle' && drawingPoints.length === 0 && 'Center'}
                {drawingMode === 'circle' && drawingPoints.length === 1 && 'Edge'}
              </div>
              {drawingMode === 'polygon' && (
                <Button 
