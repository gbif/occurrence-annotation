...
import { Trash2, ChevronLeft, ChevronRight, MessageSquare, Loader2, ThumbsUp, ThumbsDown, Minus, Plus, Edit } from 'lucide-react';
...
interface AnnotationRulesProps {
  ...
  onEditRule?: (multiPolygon: MultiPolygon) => void; // Add callback for editing rules
  onRuleDeleted?: () => void; // Add callback for when a rule is deleted during editing
...
export function AnnotationRules({ 
  ...
  const handleEditRule = async (ruleId: number) => {
    if (!onEditRule) return;
    
    const rule = rules.find(r => r.id === ruleId);
    if (!rule || !rule.multiPolygon) {
      toast.error('Rule or polygon data not found');
      return;
    }
    ...
    try {
      ...
      // First, call the parent callback to set the polygon as active
      onEditRule(rule.multiPolygon);
      
      // Then try to delete the old rule from GBIF
      let deletionSuccessful = false;
      const authHeader = getBasicAuthHeader();
      
      if (!authHeader) {
        toast.error('Please log in to GBIF to delete the rule from server, but proceeding with local edit...');
      } else {
        try {
          console.log(`Attempting to delete rule ${ruleId}...`);
          const response = await fetch(`https://api.gbif.org/v1/annotation/rule/${ruleId}`, {
            method: 'DELETE',
            headers: {
          ...
          if (response.ok) {
            ...
            console.log(`Rule ${ruleId} not found (404) - it may have already been deleted`);
            toast.warning('Rule not found on server (may have been already deleted)');
            deletionSuccessful = true; // Treat as success since rule is gone
          } else if (response.status === 401) {
            toast.error('Unauthorized - please check your GBIF credentials');
          } else if (response.status === 403) {
            toast.error('Forbidden - you may not have permission to delete this rule');
          } else {
            toast.error(`Failed to delete rule from server (${response.status}), but proceeding with local edit`);
          }
        } catch (fetchError) {
          console.error('Network error during deletion:', fetchError);
          toast.error('Network error while deleting rule, but proceeding with local edit');
        }
      }

      // Always remove the rule from local state, regardless of server deletion success
      // This ensures the UI is updated even if the server operation fails
      setRules(prevRules => {
        const updatedRules = prevRules.filter(r => r.id !== ruleId);
        console.log(`Updated rules count: ${updatedRules.length}`);
        onRulesLoadRef.current?.(updatedRules);
        return updatedRules;
      });
      ...
      // DON'T call onRuleDeleted() here - it would trigger a refresh that might bring the rule back
      // We only want to update local state, not refetch from server
      console.log('Rule removed from local state only - not triggering server refresh');
      
      if (deletionSuccessful) {
        toast.success('Rule loaded for editing and deleted from server. You can now modify and save the polygon.');
      } else {
        toast.success('Rule loaded for editing (removed from list). You can now modify and save the polygon.');
      }
    } catch (err) {
          ...
          return (
            <Card key={rule.id} className="p-3">
              ...
              <div className="mt-2 pt-2 border-t flex gap-1.5 items-center">
                ...
                {/* Edit button - only show for rules created by current user */}
                {currentUser && rule.createdBy === currentUser.userName && (
                  <>
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button
                          size="sm"
                          variant="outline"
                          disabled={!isLoggedIn || editingRuleId === rule.id}
                          className="h-7 w-7 p-0 text-blue-600 border-blue-300 hover:bg-blue-50 hover:border-blue-400 disabled:opacity-50"
                          title={!isLoggedIn ? "Login to GBIF to edit rules" : "Edit rule"}
                        >
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Are you sure you want to edit this annotation rule (ID: {rule.id})?
                            <br /><br />
                            <strong>Warning:</strong> Editing this rule will:
                            <ul className="list-disc ml-4 mt-2 space-y-1">
                              <li>Delete the current rule (including all comments and votes)</li>
                              <li>Load the polygon for editing on the map</li>
                              <li>Allow you to modify and save the polygon as a new rule</li>
                            </ul>
                            <br />
