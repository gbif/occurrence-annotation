  ...
  const renderPolygonPreview = (multiPolygon: MultiPolygon | undefined, annotation: string, onClick?: () => void) => {
    ...
    return (
      <svg 
        ...
        onClick={onClick}
      >
        {/* World map background - scaled to current view bounds */}
        <g opacity="0.15" fill="#6b7280">
          {/* Draw world continents if they intersect with current view */}
          {(() => {
            // Check if global view or if continents would be visible
            const isGlobalView = (viewMaxLng - viewMinLng) > 180 || (viewMaxLat - viewMinLat) > 90;
            
            if (!isGlobalView) return null; // Only show for global/large views
            
            // Scale world coordinates to current view
            const worldToSvg = (lat: number, lng: number) => {
              const x = ((lng - viewMinLng) / (viewMaxLng - viewMinLng)) * svgSize;
              const y = ((viewMaxLat - lat) / (viewMaxLat - viewMinLat)) * svgSize;
              return [x, y];
            };
            ...
            const continents = [
              // North America
              [[60, -140], [70, -100], [50, -60], [25, -80], [20, -100], [45, -160]],
              // South America
              [[10, -80], [5, -35], [-20, -40], [-55, -65], [-20, -85]],
              // Europe  
              [[70, -10], [60, 40], [40, 30], [50, 0]],
              // Africa
              [[35, 0], [35, 50], [0, 45], [-35, 20], [-35, 10], [15, 0]],
              // Asia
              [[70, 40], [70, 180], [20, 140], [10, 60]],
              // Australia
              [[-10, 110], [-40, 155], [-40, 110]]
            ];
            
            return continents.map((continent, i) => {
              const points = continent.map(([lat, lng]) => worldToSvg(lat, lng)).join(' ');
