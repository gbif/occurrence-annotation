...
export function MapComponent({
  ...
  useEffect(() => {
    if (!isMapDragging.current && !dragStartRef.current) {
      stableMapRef.current = { center, zoom };
      console.log('📍 Stable map ref updated via useEffect:', { center, zoom });
    } else {
    ...
    >
      <Map
        ...
        onBoundsChanged={({ center, zoom: newZoom }) => {
          ...
          // Only update stable ref if not dragging - use longer delay for better drag end detection
          if (!isMapDragging.current) {
            setTimeout(() => {
              stableMapRef.current = { center: clampedCenter, zoom: newZoom };
              console.log('🗺️ Non-drag bounds change - stable ref updated immediately');
            }, 50);
          } else {
            // During drag, schedule an update for when drag ends
            setTimeout(() => {
              if (!isMapDragging.current) {
                stableMapRef.current = { center: clampedCenter, zoom: newZoom };
                dragStartRef.current = null;
                console.log('🗺️ Drag ended - stable ref updated with final position');
              }
            }, 200);
          }
        }}
