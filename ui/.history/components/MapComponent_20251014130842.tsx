...
export function MapComponent({
  ...
  useEffect(() => {
    if (mapContainerRef.current) {
      ...
      const handleWheel = (e: WheelEvent) => {
        // Trigger zoom state for any wheel movement (be more responsive)
        setIsZooming(true);
        ...
        // Very short timeout for immediate responsiveness
        zoomTimeoutRef.current = window.setTimeout(() => {
          setIsZooming(false);
        }, 100); // Much shorter timeout for instant response
      };
  ...
  useEffect(() => {
    // Disable tile clearing during zoom for maximum smoothness
    // Only clear tiles when species changes or map is reset
    // This prevents unnecessary tile reloading during zoom operations
  }, [zoom]);
