...
export function AnnotationRules({ 
  ...
  const handleEditRule = async (ruleId: number) => {
    ...
    try {
      ...
      if (!authHeader) {
        toast.error('Please log in to GBIF to edit rules');
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          // Rule not found - it may have already been deleted, but we can still proceed
          console.log(`Rule ${ruleId} not found (404) - it may have already been deleted`);
          toast.warning('Rule not found on server (may have been already deleted), but proceeding with edit...');
        } else {
          throw new Error(`Failed to delete rule (${response.status})`);
        }
      } else {
      ...
      // Remove the deleted rule from state (whether it was deleted now or was already gone)
      setRules(prevRules => {
        const updatedRules = prevRules.filter(r => r.id !== ruleId);
      ...
      // Notify parent that a rule was deleted during editing
      if (onRuleDeleted) {
        console.log('Calling onRuleDeleted callback');
        onRuleDeleted();
      }
      
      toast.success('Rule loaded for editing. You can now modify and save the polygon.');
    } catch (err) {
