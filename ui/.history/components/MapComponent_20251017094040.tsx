  ...
  useEffect(() => {
    if (mapContainerRef.current) {
      ...
      const handleWheel = (e: WheelEvent) => {
        // Check if we're at zoom limits to avoid unnecessary state changes
        const atMaxZoom = zoom >= 18;
        const atMinZoom = zoom <= 1;
        const zoomingOut = e.deltaY > 0;
        const zoomingIn = e.deltaY < 0;
        
        // Don't set zooming state if we're at limits and trying to zoom beyond them
        if ((atMaxZoom && zoomingIn) || (atMinZoom && zoomingOut)) {
          console.log('🗺️ WHEEL - At zoom limit, ignoring:', {
            currentZoom: zoom,
            atMaxZoom,
            atMinZoom,
            direction: zoomingOut ? 'out' : 'in',
            status: 'No state change - at zoom boundary'
          });
          return;
        }
    ...
    >
      <Map
        ...
        onBoundsChanged={({ center: newCenter, zoom: newZoom }) => {
          // Check if zoom actually changed significantly (increased threshold)
          const zoomChanged = Math.abs(newZoom - zoom) > 0.5;
          
          // Check if center actually changed significantly
          const centerChanged = Math.abs(newCenter[0] - center[0]) > 0.001 || 
                               Math.abs(newCenter[1] - center[1]) > 0.001;
          
          // Only process if there's an actual change
          if (!zoomChanged && !centerChanged) {
            console.log('🗺️ BOUNDS CHANGED - NO SIGNIFICANT CHANGE:', {
              zoomDiff: Math.abs(newZoom - zoom),
              centerDiff: [Math.abs(newCenter[0] - center[0]), Math.abs(newCenter[1] - center[1])],
              status: 'Skipping update - no significant change detected'
            });
            return; // Prevent unnecessary updates when at zoom limits
          }
          ...
          const clampedCenter: [number, number] = [
            clampLatitude(newCenter[0]), // Clamp latitude
            newCenter[1] // Longitude can wrap around
          ];
