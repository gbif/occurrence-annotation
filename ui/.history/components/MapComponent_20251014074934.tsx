...
export function MapComponent({
  ...
  const createPolygonTile = (tileX: number, tileY: number, tileZ: number): string => {
    ...
    // Expand tile bounds slightly to catch polygons that cross tile boundaries
    const latExpansion = (nwLat - seLat) * 0.1; // 10% expansion
    const lngExpansion = (seLng - nwLng) * 0.1; // 10% expansion
    const expandedNwLat = nwLat + latExpansion;
    const expandedNwLng = nwLng - lngExpansion;
    const expandedSeLat = seLat - latExpansion;
    const expandedSeLng = seLng + lngExpansion;
    ...
    // Check if a polygon intersects with this tile (including expansion)
    const polygonIntersectsTile = (coords: [number, number][]): boolean => {
      return coords.some(([lat, lng]) => 
        lat >= expandedSeLat && lat <= expandedNwLat && 
        lng >= expandedNwLng && lng <= expandedSeLng
      );
    };
    ...
    annotationRules.forEach(rule => {
      ...
      rule.multiPolygon.polygons.forEach(polygonWithHoles => {
        // Only draw if polygon intersects this tile
  ...
  useEffect(() => {
    ...
    const tileZoom = Math.min(10, Math.max(4, Math.round(zoom - 2))); // Use a more conservative zoom level
    const [centerX, centerY] = latLngToTileWebMercator(center[0], center[1], tileZoom);
    
    // Generate more tiles to ensure coverage
    const tilesX = Math.min(8, Math.ceil(mapSize.width / 256) + 3);
    const tilesY = Math.min(8, Math.ceil(mapSize.height / 256) + 3);
