...
import { Trash2, Repeat, Edit2, Upload, Loader2, Plus, Check, MessageSquare } from 'lucide-react';
          ...
          {onAddToMultiPolygon && (
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>{isAddingTo ? "Stop adding polygons" : "Add polygon"}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
                ...
                {JSON.parse(localStorage.getItem('polygonComments') || '[]')
                  ...
                  .map((comment: any, index: number) => (
                    <p key={index} className="text-xs text-amber-700 bg-white px-2 py-1 rounded">
                      "{comment.comment}"{comment.user && comment.user !== 'Anonymous' ? ` - ${comment.user}` : ''}
                    </p>
                  ))}
              </div>
            </div>
