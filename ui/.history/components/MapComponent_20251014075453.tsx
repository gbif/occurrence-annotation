...
export function MapComponent({
  ...
  useEffect(() => {
    ...
    // Use a fixed tile zoom level to prevent splitting during zoom changes
    const tileZoom = Math.min(10, Math.max(3, Math.round(zoom - 1))); // More conservative zoom
    const [centerX, centerY] = latLngToTileWebMercator(center[0], center[1], tileZoom);
    
    // Calculate bounds for all polygons to ensure coverage
    let minLat = center[0], maxLat = center[0];
    let minLng = center[1], maxLng = center[1];
    
    // Find bounds of all annotation rules
    annotationRules.forEach(rule => {
    ...
    // Find bounds of all saved polygons
    savedPolygons.forEach(polygonData => {
      const polygonParts: [number, number][][] = polygonData.isMultiPolygon 
        ? (polygonData.coordinates as [number, number][][])
        : [polygonData.coordinates as [number, number][]];
    ...
    // Convert polygon bounds to tile coordinates
    const [minTileX, maxTileY] = latLngToTileWebMercator(maxLat, minLng, tileZoom);
    const [maxTileX, minTileY] = latLngToTileWebMercator(minLat, maxLng, tileZoom);
    
    // Calculate viewport-based tile coverage as fallback
    const viewportTilesX = Math.min(12, Math.ceil(mapSize.width / 256) + 4);
    const viewportTilesY = Math.min(12, Math.ceil(mapSize.height / 256) + 4);
    
    // Use the larger of polygon bounds or viewport coverage
    const tilesMinX = Math.max(0, Math.min(centerX - Math.floor(viewportTilesX / 2), minTileX - 1));
    const tilesMaxX = Math.min(Math.pow(2, tileZoom) - 1, Math.max(centerX + Math.ceil(viewportTilesX / 2), maxTileX + 1));
    const tilesMinY = Math.max(0, Math.min(centerY - Math.floor(viewportTilesY / 2), minTileY - 1));
    const tilesMaxY = Math.min(Math.pow(2, tileZoom) - 1, Math.max(centerY + Math.ceil(viewportTilesY / 2), maxTileY + 1));
    
    const newPolygonTiles = [];
    for (let x = tilesMinX; x <= tilesMaxX; x++) {
      for (let y = tilesMinY; y <= tilesMaxY; y++) {
        const anchor = tileToLatLngWebMercator(x, y, tileZoom);
        const dataUrl = createPolygonTile(x, y, tileZoom);
        
        if (dataUrl) {
          newPolygonTiles.push({ x, y, z: tileZoom, anchor, dataUrl });
        }
      }
    }
    
    console.log(`📍 Generated ${newPolygonTiles.length} polygon tiles (zoom: ${tileZoom}, bounds: ${tilesMinX}-${tilesMaxX}, ${tilesMinY}-${tilesMaxY})`);
    setPolygonTiles(newPolygonTiles);
  }, [zoom, center, mapSize, annotationRules, savedPolygons, editingPolygonId]);
