...
// GBIF base map tile provider (EPSG:4326)
// Note: pigeon-maps expects Web Mercator tiles, so we need to transform coordinates
const gbifTileProvider = (x: number, y: number, z: number) => {
  // pigeon-maps sends Web Mercator tile coordinates, but GBIF uses EPSG:4326
  // We need to convert from Web Mercator tile coordinates to EPSG:4326 tile coordinates
  
  // Get the lat/lng bounds of the Web Mercator tile
  const [nwLat, nwLng] = tileToLatLngWebMercator(x, y, z);
  const [seLat, seLng] = tileToLatLngWebMercator(x + 1, y + 1, z);
  
  // Get the center of the tile
  const centerLat = (nwLat + seLat) / 2;
  const centerLng = (nwLng + seLng) / 2;
  
  // Convert to EPSG:4326 tile coordinates
  const [epsg4326X, epsg4326Y] = latLngToTileEPSG4326(centerLat, centerLng, z);
  
  const url = `https://tile.gbif.org/4326/omt/${z}/${epsg4326X}/${epsg4326Y}@1x.png?style=gbif-geyser-en&srs=EPSG%3A4326`;
  console.log(`🗺️ Tile transform: WebMercator(${x},${y},${z}) -> EPSG4326(${epsg4326X},${epsg4326Y},${z}) | Center: ${centerLat.toFixed(2)},${centerLng.toFixed(2)}`);
  return url;
};
...
export function MapComponent({
  ...
  useEffect(() => {
    ...
    const [centerX, centerY] = latLngToTileEPSG4326(center[0], center[1], tileZoom);
    ...
    for (let dx = -Math.floor(tilesX / 2); dx <= Math.ceil(tilesX / 2); dx++) {
      for (let dy = -Math.floor(tilesY / 2); dy <= Math.ceil(tilesY / 2); dy++) {
        ...
        if (x >= 0 && x < maxTile && y >= 0 && y < maxTile) {
          ...
          const anchor = tileToLatLngEPSG4326(x, y, tileZoom);
