...
export function MapComponent({
  ...
  const createPolygonTile = (tileX: number, tileY: number, tileZ: number): string => {
    ...
    // Draw annotation rules (simplified approach - always draw if they exist)
    ...
    // Draw saved polygons (simplified approach - always draw if they exist)
    ...
    // Only return the data URL if we actually drew something
    if (drewSomething) {
      console.log(`🎨 Drew content on tile ${tileX},${tileY} at zoom ${tileZ}`);
  ...
  useEffect(() => {
    ...
    console.log('🔄 Regenerating polygon tiles...', { 
      annotationRulesCount: annotationRules.length, 
      savedPolygonsCount: savedPolygons.length,
      center 
    });
    ...
    console.log('🗂️ Tile bounds:', { tilesMinX, tilesMaxX, tilesMinY, tilesMaxY });
    ...
    for (let x = tilesMinX; x < tilesMinX + actualTilesX; x++) {
      for (let y = tilesMinY; y < tilesMinY + actualTilesY; y++) {
        ...
        if (dataUrl && dataUrl !== 'data:,') { // Check for valid canvas data
  ...
  }, [center, annotationRules, savedPolygons, editingPolygonId]); // Simplified dependencies
    ...
    >
      ...
      >
        ...
        {polygonTiles.map((tile) => {
          ...
          console.log(`🖼️ Rendering polygon tile ${tile.x},${tile.y} at zoom ${tile.z}, scale: ${scaleFactor}, size: ${displaySize}px`);
          ...
          return (
            ...
            >
              <div style={{ 
                pointerEvents: 'none',
                border: '1px solid red', // DEBUG: Add border to see tile boundaries
                backgroundColor: 'rgba(255,0,0,0.1)', // DEBUG: Add background to see tile placement
              }}>
                <img
                  src={tile.dataUrl}
                  alt=""
                  ...
                  onLoad={() => console.log(`✅ Polygon tile ${tile.x},${tile.y} loaded successfully`)}
                  onError={() => console.log(`❌ Polygon tile ${tile.x},${tile.y} failed to load`)}
                />
              </div>
            </Overlay>
          );
      ...
      {mapSize.width > 0 && mapSize.height > 0 && (
        ...
        >
          ...
          >
            {/* Canvas tiles are working - disabling SVG again
            ...
            {/* Canvas tiles are now working - commenting out saved polygons SVG rendering again
