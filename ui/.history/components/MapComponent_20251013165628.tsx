...
export function MapComponent({
    ...
    for (let dx = -Math.floor(tilesX / 2); dx <= Math.ceil(tilesX / 2); dx++) {
      for (let dy = -Math.floor(tilesY / 2); dy <= Math.ceil(tilesY / 2); dy++) {
        ...
        if (x >= 0 && x < maxTile && y >= 0 && y < maxTile) {
          ...
          const url = `https://api.gbif.org/v2/map/occurrence/adhoc/${tileZoom}/${x}/${y}@2x.png?srs=EPSG:3857&style=scaled.circles&taxonKey=${selectedSpecies.key}&hasGeospatialIssue=false`;
  ...
  return (
    <div 
      ref={mapContainerRef} 
      className={`relative w-full h-full overflow-hidden ${isInvestigateMode ? 'cursor-crosshair' : ''}`}
