...
function PolygonCard({ 
  ...
  // Auto-invert hack: For polygons marked as inverted but MiniMapPreview not showing correctly
  useEffect(() => {
    // Only trigger auto-invert for polygons that are marked as inverted
    // This helps fix the mini map display issue
    if (polygon.inverted === true) {
      console.log('🔄 Polygon is marked as inverted, checking mini map display:', {
        id: polygon.id,
        inverted: polygon.inverted,
        isMultiPolygon: polygon.isMultiPolygon
      });
      
      // For inverted multipolygons that aren't displaying correctly in mini map,
      // we could trigger a re-render or other fixes here
      // For now, just log to help with debugging
    }
  }, [polygon.id, polygon.inverted, polygon.isMultiPolygon]);
