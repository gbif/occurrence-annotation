...
// Coordinate correction utility - addresses potential misalignment issues
const correctCoordinates = (coords: [number, number][]): [number, number][] => {
  return coords.map(([lat, lng]) => {
    // Ensure coordinates are within valid ranges
    const correctedLat = Math.max(-85.0511287798, Math.min(85.0511287798, lat));
    const correctedLng = Math.max(-180, Math.min(180, lng));
    
    // If there's a systematic offset detected, you can apply correction here
    // For example, if there's a consistent 5-degree offset:
