  ...
  return (
    <div className={`border border-gray-200 rounded overflow-hidden ${className}`}
      ...
      <ComposableMap
        ...
        projectionConfig={{
          scale: Math.min(width, height) * 0.18, // Increased scale to fill the space better
          center: [0, 10], // Slightly south to center continents better
        }}
        width={width}
        height={height}
        style={{
        ...
        {polygonCoordinates.map((ring, index) => {
          ...
          return (
            <path
              ...
              strokeWidth={0.8} // Reduced from 1.5 for mini map
              style={{ outline: "none" }}
            />
          );
        })}
