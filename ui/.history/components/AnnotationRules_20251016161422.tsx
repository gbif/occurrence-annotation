...
import { AlertCircle, CheckCircle, XCircle, Trash2, ChevronLeft, ChevronRight, MessageSquare, Loader2, Pencil, ThumbsUp, ThumbsDown, Minus, Plus } from 'lucide-react';
...
export function AnnotationRules({ 
  ...
  const shouldUseGlobeView = (multiPolygon: MultiPolygon): boolean => {
    const allCoords = multiPolygon.polygons.flatMap(poly => 
  ...
  const renderPolygonPreview = (multiPolygon: MultiPolygon | undefined, annotation: string, onClick?: () => void) => {
    ...
    if (shouldUseGlobeView(multiPolygon)) {
      const globeElement = (
        <GlobeView 
          multiPolygon={multiPolygon}
    ...
    // Use regular 2D preview for larger polygons
