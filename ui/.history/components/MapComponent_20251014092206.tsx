...
export function MapComponent({
  ...
  useEffect(() => {
    ...
    // Only clear tiles if zoom changed by more than 0.5 levels to reduce flicker
  ...
  useEffect(() => {
    ...
    // Update when center changes (after pan/drag)
    else if (stableCenter.current[0] !== center[0] || stableCenter.current[1] !== center[1]) {
      stableCenter.current = center;
    }
  }, [center, zoom, gbifTiles.length]);
  ...
  const latLngToPixel = (lat: number, lng: number): [number, number] => {
    // Use stable references during map operations to prevent jitter
    const currentCenter = stableCenter.current;
    const currentZoom = stableZoom.current;
    
    const [worldX, worldY] = latLngToWorld(lat, lng, currentZoom);
    const [centerWorldX, centerWorldY] = latLngToWorld(currentCenter[0], currentCenter[1], currentZoom);
