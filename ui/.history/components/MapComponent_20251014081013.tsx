...
export function MapComponent({
  ...
  const [polygonTiles, setPolygonTiles] = useState<PolygonTile[]>([]);
  const [lastTileCenter, setLastTileCenter] = useState<[number, number]>([0, 0]);
  ...
  // Clear GBIF tiles when zoom level changes significantly (not just on wheel events)
  useEffect(() => {
    const previousZoom = stableZoom.current;
    const zoomDifference = Math.abs(zoom - previousZoom);
    
    // Only clear tiles if zoom changed by more than 1 full level to reduce flicker
    // This allows for smoother intermediate zoom levels
    if (zoomDifference > 1.0) {
      setGbifTiles([]);
    }
  }, [zoom]);

  // Create canvas-based polygon tiles for stable rendering
  const createPolygonTile = (tileX: number, tileY: number, tileZ: number): string => {
    ...
    const [nwLat, nwLng] = tileToLatLngWebMercator(tileX, tileY, tileZ);
    const [seLat, seLng] = tileToLatLngWebMercator(tileX + 1, tileY + 1, tileZ);

    // Convert lat/lng coordinates to tile pixel coordinates
    const convertToTilePixel = (lat: number, lng: number): [number, number] => {
      const x = ((lng - nwLng) / (seLng - nwLng)) * 256;
      const y = ((lat - nwLat) / (seLat - nwLat)) * 256;
      return [x, y];
    };

    // Check if a polygon intersects with this tile using bounding box
    const polygonIntersectsTile = (coords: [number, number][]): boolean => {
      ...
      // Check if polygon bounding box intersects tile bounds
      return !(maxLat < seLat || minLat > nwLat || maxLng < nwLng || minLng > seLng);
    };

    // Draw annotation rules
    annotationRules.forEach(rule => {
      if (!rule.multiPolygon) return;

      const colors = {
        SUSPICIOUS: { fill: 'rgba(239, 68, 68, 0.15)', stroke: 'rgba(220, 38, 38, 1)' },
        INTRODUCED: { fill: 'rgba(34, 197, 94, 0.15)', stroke: 'rgba(22, 163, 74, 1)' },
        MANAGED: { fill: 'rgba(59, 130, 246, 0.15)', stroke: 'rgba(37, 99, 235, 1)' },
        FORMER: { fill: 'rgba(147, 51, 234, 0.15)', stroke: 'rgba(126, 34, 206, 1)' },
        VAGRANT: { fill: 'rgba(234, 88, 12, 0.15)', stroke: 'rgba(194, 65, 12, 1)' },
      };
      
      const colorSet = colors[rule.annotation as keyof typeof colors] || colors.SUSPICIOUS;

      rule.multiPolygon.polygons.forEach(polygonWithHoles => {
        // Check if this polygon intersects with the tile
        if (!polygonIntersectsTile(polygonWithHoles.outer)) return;

        // Draw outer polygon
        ctx.beginPath();
        const outerPixels = polygonWithHoles.outer.map(([lat, lng]) => convertToTilePixel(lat, lng));
        
        if (outerPixels.length > 0) {
          ctx.moveTo(outerPixels[0][0], outerPixels[0][1]);
          outerPixels.slice(1).forEach(([x, y]) => ctx.lineTo(x, y));
          ctx.closePath();

          // Draw holes that intersect this tile
          polygonWithHoles.holes.forEach(hole => {
            if (polygonIntersectsTile(hole)) {
              const holePixels = hole.map(([lat, lng]) => convertToTilePixel(lat, lng));
              if (holePixels.length > 0) {
                ctx.moveTo(holePixels[0][0], holePixels[0][1]);
                holePixels.slice(1).forEach(([x, y]) => ctx.lineTo(x, y));
                ctx.closePath();
              }
            }
          });
    ...
    // Draw saved polygons
    savedPolygons.forEach(polygonData => {
      const colors = {
        SUSPICIOUS: { fill: 'rgba(239, 68, 68, 0.1)', stroke: 'rgba(220, 38, 38, 1)' },
        INTRODUCED: { fill: 'rgba(34, 197, 94, 0.1)', stroke: 'rgba(22, 163, 74, 1)' },
        MANAGED: { fill: 'rgba(59, 130, 246, 0.1)', stroke: 'rgba(37, 99, 235, 1)' },
        FORMER: { fill: 'rgba(147, 51, 234, 0.1)', stroke: 'rgba(126, 34, 206, 1)' },
        VAGRANT: { fill: 'rgba(234, 88, 12, 0.1)', stroke: 'rgba(194, 65, 12, 1)' },
      };
      
      const color = colors[polygonData.annotation as keyof typeof colors] || colors.SUSPICIOUS;
      ...
      polygonParts.forEach(polyCoords => {
        // Check if this polygon part intersects with the tile
        if (!polygonIntersectsTile(polyCoords)) return;

        const pixels = polyCoords.map(([lat, lng]) => convertToTilePixel(lat, lng));
  ...
  // Generate polygon tiles when needed
  useEffect(() => {
    if (!mapSize.width || !mapSize.height || (!annotationRules.length && !savedPolygons.length)) {
      setPolygonTiles([]);
      return;
    }

    // Use a FIXED tile zoom level that never changes - this prevents grid shifts
    const FIXED_TILE_ZOOM = 8; // Fixed at zoom 8 for stability
    
    // Only regenerate tiles when center moves significantly or polygons change
    const centerDistance = Math.sqrt(
      Math.pow(center[0] - lastTileCenter[0], 2) + 
      Math.pow(center[1] - lastTileCenter[1], 2)
    );
    
    // Threshold for regenerating tiles (about 0.1 degrees)
    const REGENERATION_THRESHOLD = 0.1;
    
    // Skip regeneration if center hasn't moved much and polygons haven't changed
    if (centerDistance < REGENERATION_THRESHOLD && polygonTiles.length > 0) {
      return;
    }
    
    setLastTileCenter(center);
    
    // Calculate viewport bounds at the fixed zoom level
    const latRange = 180 / Math.pow(2, FIXED_TILE_ZOOM) * 6; // Cover more area
    const lngRange = 360 / Math.pow(2, FIXED_TILE_ZOOM) * 6;
    
    const viewMinLat = center[0] - latRange;
    const viewMaxLat = center[0] + latRange;
    const viewMinLng = center[1] - lngRange;
    const viewMaxLng = center[1] + lngRange;
    
    // Find bounds of all polygons
    let minLat = viewMinLat, maxLat = viewMaxLat;
    let minLng = viewMinLng, maxLng = viewMaxLng;
    
    // Expand bounds to include all annotation rules
    annotationRules.forEach(rule => {
      if (rule.multiPolygon) {
        rule.multiPolygon.polygons.forEach(polygonWithHoles => {
          polygonWithHoles.outer.forEach(([lat, lng]) => {
            minLat = Math.min(minLat, lat);
            maxLat = Math.max(maxLat, lat);
            minLng = Math.min(minLng, lng);
            maxLng = Math.max(maxLng, lng);
    ...
    // Expand bounds to include all saved polygons
    savedPolygons.forEach(polygonData => {
      const polygonParts: [number, number][][] = polygonData.isMultiPolygon 
        ? (polygonData.coordinates as [number, number][][])
        : [polygonData.coordinates as [number, number][]];
    ...
    // Convert bounds to tile coordinates at fixed zoom
    const [minTileX, maxTileY] = latLngToTileWebMercator(maxLat, minLng, FIXED_TILE_ZOOM);
    const [maxTileX, minTileY] = latLngToTileWebMercator(minLat, maxLng, FIXED_TILE_ZOOM);
    
    // Add padding around polygon bounds
    const paddingTiles = 3;
    const tilesMinX = Math.max(0, minTileX - paddingTiles);
    const tilesMaxX = Math.min(Math.pow(2, FIXED_TILE_ZOOM) - 1, maxTileX + paddingTiles);
    const tilesMinY = Math.max(0, minTileY - paddingTiles);
    const tilesMaxY = Math.min(Math.pow(2, FIXED_TILE_ZOOM) - 1, maxTileY + paddingTiles);
    
    // Limit maximum number of tiles for performance
    const maxTilesPerDimension = 25;
    const actualTilesX = Math.min(maxTilesPerDimension, tilesMaxX - tilesMinX + 1);
    const actualTilesY = Math.min(maxTilesPerDimension, tilesMaxY - tilesMinY + 1);
    
    const newPolygonTiles: PolygonTile[] = [];
    for (let x = tilesMinX; x < tilesMinX + actualTilesX; x++) {
      for (let y = tilesMinY; y < tilesMinY + actualTilesY; y++) {
        const anchor = tileToLatLngWebMercator(x, y, FIXED_TILE_ZOOM);
        const dataUrl = createPolygonTile(x, y, FIXED_TILE_ZOOM);
        
        if (dataUrl) {
          newPolygonTiles.push({ x, y, z: FIXED_TILE_ZOOM, anchor, dataUrl });
        }
      }
    }
    
    console.log(`📍 Generated ${newPolygonTiles.length} polygon tiles at FIXED zoom ${FIXED_TILE_ZOOM} (${actualTilesX}x${actualTilesY})`);
    setPolygonTiles(newPolygonTiles);
  }, [center, annotationRules, savedPolygons, editingPolygonId, lastTileCenter, polygonTiles.length]); // Removed zoom dependency!
  ...
  const latLngToStablePixel = (lat: number, lng: number): [number, number] => {
    // Use current values during zoom for smooth transitions, stable values otherwise
    const useCurrentZoom = isZooming;
    const currentZoomValue = useCurrentZoom ? zoom : stableZoom.current;
    const currentCenterValue = useCurrentZoom ? center : stableCenter.current;
    
    const [worldX, worldY] = latLngToWorld(lat, lng, currentZoomValue);
    const [centerWorldX, centerWorldY] = latLngToWorld(currentCenterValue[0], currentCenterValue[1], currentZoomValue);
    ...
    >
      ...
      >
        ...
        {gbifTiles.map((tile) => {
          ...
          // Smooth opacity transition during zoom
          const opacity = isZooming ? 0.7 : 1.0;
              ...
              <div style={{ 
                ...
                <img
                  ...
                  style={{
                    ...
                    transform: isZooming ? 'scale(1.02)' : 'scale(1)',
                    imageRendering: 'auto',
                  }}
                  onLoad={(e) => {
                    // Smooth fade-in when tile loads
        ...
        {polygonTiles.map((tile) => {
          ...
          return (
            <Overlay 
              key={`polygon-${tile.z}-${tile.x}-${tile.y}`} 
              anchor={[nwLat, nwLng]} 
              offset={[0, 0]}
            >
              <div style={{ 
      ...
      {mapSize.width > 0 && mapSize.height > 0 && (
        ...
        >
          ...
          <g
            style={{
              transform: mapTransform && !isZooming ? `translate(${calculateTransformOffset()[0]}px, ${calculateTransformOffset()[1]}px)` : 'none',
              transformOrigin: '0 0',
              transition: isZooming ? 'none' : 'transform 0.1s ease-out',
            }}
          >
            {/* Annotation rule polygons - now rendered via canvas tiles for stability
            {showAnnotationRules && annotationRules.map((rule) => {
              ...
              return (
                <path
                  ...
                  fillRule="evenodd"
                  style={{
                    transition: isZooming ? 'none' : 'all 0.1s ease-out'
                  }}
                />
              );
            })} */}

            {/* Saved polygons - now rendered via canvas tiles for stability
