...
export function AnnotationRules({ 
    ...
    const fetchRules = async () => {
      ...
      try {
        ...
        const rulesWithCoords = uniqueRules.map(rule => {
          // Debug coordinate transformation for the first rule
          if (rule.geometry && uniqueRules.indexOf(rule) === 0) {
            console.log('🔍 DEBUGGING FIRST RULE COORDINATES:');
            console.log('Original WKT:', rule.geometry.substring(0, 200) + '...');
            
            // Extract first few coordinate pairs for analysis
            const coordsMatch = rule.geometry.match(/\(\(([^)]+)\)/);
            if (coordsMatch) {
              const coordPairs = coordsMatch[1].split(',').slice(0, 3); // First 3 pairs
              coordPairs.forEach((pair, idx) => {
                const parts = pair.trim().split(/\s+/).map(Number);
                const [lng, lat] = parts; // WKT format
                console.log(`Coord ${idx + 1}: WKT(${lng}, ${lat}) -> Map([${lat}, ${lng}])`);
              });
            }
          }
  ...
  const handleAddComment = async (ruleId: number) => {
    ...
    try {
      ...
      // Refresh comments for this rule
      setComments(prev => {
        const newMap = new Map(prev);
      ...
      await fetchCommentCount(ruleId);
      if (openComments.has(ruleId)) {
        await fetchComments(ruleId);
      }
    } catch (err) {
          ...
          return (
            <Card key={rule.id} className="p-3">
              ...
              <div className="mt-2 pt-2 border-t flex gap-1.5">
                ...
                <Dialog 
                  open={addCommentDialogOpen === rule.id} 
                  onOpenChange={(open) => {
                    setAddCommentDialogOpen(open ? rule.id : null);
                ...
                >
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Add a comment to this annotation rule (ID: {rule.id})
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                      <div className="space-y-2">
                        <Label htmlFor={`comment-${rule.id}`}>Comment</Label>
                        <Textarea
                          id={`comment-${rule.id}`}
