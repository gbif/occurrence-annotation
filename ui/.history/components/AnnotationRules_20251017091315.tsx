...
export function AnnotationRules({ 
  ...
  const renderPolygonPreview = (multiPolygon: MultiPolygon | undefined, annotation: string, onClick?: () => void) => {
    ...
    let isOriginallyInverted = false;

    if (multiPolygon.polygons.length === 1) {
      const polygon = multiPolygon.polygons[0];
      isOriginallyInverted = isInvertedPolygon(polygon);
      
      if (isOriginallyInverted && polygon.holes && polygon.holes.length > 0) {
        // For inverted polygons, use all holes as the actual polygons
        // But don't set isInverted=true because the holes represent the normal areas
        ...
        isInverted = false; // Don't invert in preview since holes are the actual areas
      } else {
