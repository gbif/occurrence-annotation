...
export function MapComponent({
  ...
  const handleCurrentEdgeClick = (e: React.MouseEvent, edgeStartIndex: number) => {
    ...
    // Check if the midpoint would be within bounds
    if (!isWithinBounds(midLat, midLng)) {
      toast.error('Cannot add vertex outside map boundaries');
      return;
    }
    ...
    if (polygon.isMultiPolygon) {
      ...
      const newCoords = coords.map(part => {
        ...
        for (let i = 0; i < part.length; i++) {
          ...
          // Only add midpoint if it's within bounds
          if (isWithinBounds(midLat, midLng)) {
