...
// GBIF base map tile provider - using Web Mercator (EPSG:3857) for compatibility
// This ensures proper compatibility with pigeon-maps which expects Web Mercator tiles
const gbifTileProvider = (x: number, y: number, z: number) => {
  // Use GBIF's Web Mercator tiles for proper pigeon-maps compatibility
  const url = `https://tile.gbif.org/3857/omt/${z}/${x}/${y}@1x.png?style=gbif-geyser-en`;
  console.log(`🗺️ Loading GBIF Web Mercator tile: z=${z}, x=${x}, y=${y}`);
  return url;
};
