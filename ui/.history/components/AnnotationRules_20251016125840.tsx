...
export function AnnotationRules({ 
  ...
  const isPolygonSmall = (multiPolygon: MultiPolygon): boolean => {
    const allCoords = multiPolygon.polygons.flatMap(poly => 
      [poly.outer, ...poly.holes].flat()
    );
  ...
  const renderWorldMapContext = (multiPolygon: MultiPolygon, annotation: string) => {
    if (!isPolygonSmall(multiPolygon)) return null;
    
    const [centerLat, centerLng] = calculatePolygonCenter(multiPolygon);
    
    // Convert lat/lng to world map coordinates (higher resolution)
    const mapWidth = 120;
    const mapHeight = 60;
    ...
    return (
      <div className="mt-1">
        ...
        >
          {/* Higher detail continent outlines */}
          <g fill="#94a3b8" opacity="0.6" stroke="#64748b" strokeWidth="0.3">
            {/* North America */}
            <path d="M5 20 L30 15 L40 25 L35 35 L20 40 L5 35 Z M32 8 L38 5 L42 8 L40 12 L35 15 L30 12 Z"/>
            {/* South America */}
            <path d="M20 40 L30 38 L35 50 L30 60 L20 58 L15 48 Z"/>
            {/* Europe */}
            <path d="M50 15 L65 20 L60 30 L50 25 Z"/>
            {/* Africa */}
            <path d="M50 30 L60 30 L65 45 L60 55 L50 50 L45 40 Z"/>
            {/* Asia */}
            <path d="M65 15 L105 20 L110 35 L100 45 L65 40 Z"/>
            {/* Australia */}
            <path d="M90 45 L105 43 L110 50 L100 55 L90 53 Z"/>
            {/* Greenland */}
            <path d="M35 5 L45 3 L48 8 L43 12 L35 10 Z"/>
          </g>
          
          {/* Grid lines for reference */}
          <g stroke="#cbd5e1" strokeWidth="0.2" opacity="0.4">
                ...
                {rule.multiPolygon && (
                  <div className="flex-shrink-0">
                    <div className="space-y-1">
                      ...
                      {renderWorldMapContext(rule.multiPolygon, rule.annotation)}
                    </div>
                  </div>
                )}
