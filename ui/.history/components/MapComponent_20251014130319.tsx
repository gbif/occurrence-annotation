...
export function MapComponent({
  ...
  useEffect(() => {
    if (mapContainerRef.current) {
      ...
      const handleWheel = () => {
        // Set zooming state immediately for responsive feedback
        setIsZooming(true);
        ...
        // Even longer timeout for maximum smoothness
        zoomTimeoutRef.current = window.setTimeout(() => {
          setIsZooming(false);
        }, 500); // Increased to 500ms for ultra-smooth experience
      };
  ...
  useEffect(() => {
    ...
    // Don't update tiles during rapid zooming to improve performance
    if (isZooming) {
      return;
    }
  ...
  useEffect(() => {
    // Only clear tiles on major zoom changes (more than 1 level difference)
    const currentTileZoom = Math.max(0, Math.min(14, Math.floor(zoom)));
    const shouldClear = Math.abs(currentTileZoom - Math.floor(zoom)) > 1;
    ...
    >
      <Map
        ...
        onBoundsChanged={({ center, zoom: newZoom }) => {
          ...
          console.log('🗺️ BOUNDS CHANGED - Test polygon should move with map:', {
            newCenter: `${JSON.stringify(clampedCenter)}`,
            newZoom: newZoom,
            testPolygonLocation: 'NYC [40.7589, -73.9851]',
            status: 'RED test polygon should stay fixed to NYC coordinates'
          });
      ...
      >
        ...
        {/* TEST POLYGON: Fixed geographic coordinates with consistent size - COMMENTED OUT */}
        {/*
          ...
          <div style={{ pointerEvents: 'none' }}>
            <svg width="400" height="400" style={{ overflow: 'visible' }}>
              {(() => {
                ...
                // Define polygon in terms of lat/lng offsets (about 0.01 degrees = ~1km)
                const testPolygonCoords: [number, number][] = [
                  [40.7589, -73.9851],        // anchor point
                  [40.7589, -73.9751],        // 100m east
                  [40.7689, -73.9751],        // 100m north, 100m east  
                  [40.7639, -73.9801],        // 50m north, 50m west (triangle point)
                  [40.7589, -73.9801],        // back to west side
                ];
                ...
                const metersPerPixel = 40075016.686 / (256 * Math.pow(2, zoom)) / Math.cos(anchorLat * Math.PI / 180);
                const textOffset = 500 / metersPerPixel; // 500m offset for text
                const fontSize = Math.max(8, Math.min(16, 12 * Math.pow(2, zoom - 10))); // Scale font with zoom
        ...
        */}
        ...
        {/* USER-DRAWN POLYGONS as Geographic Overlays */}
        ...
        {/* SAVED POLYGONS as Geographic Overlays */}
        ...
        {/* ANNOTATION RULE POLYGONS as Geographic Overlays */}
