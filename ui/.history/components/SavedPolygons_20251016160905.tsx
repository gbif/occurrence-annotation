...
// Helper function to convert coordinates to MultiPolygon format for GlobeView
function coordinatesToMultiPolygon(coordinates: [number, number][] | [number, number][][], isMultiPolygon = false): MultiPolygon {
  ...
  return {
    polygons: polygons.map(polygon => ({
      outer: polygon,
      holes: [] // SavedPolygons doesn't handle holes currently
    }))
  };
}

// Utility function to determine if a polygon should use globe view
function shouldUseGlobeView(coordinates: [number, number][] | [number, number][][], isMultiPolygon = false): boolean {
  const polygons: [number, number][][] = isMultiPolygon 
    ? (coordinates as [number, number][][])
    : [coordinates as [number, number][]];

  if (polygons.length === 0 || polygons[0].length === 0) return false;
  ...
  // Use globe view for small polygons (less than 10 degrees in any dimension)
  // These would benefit from seeing their global context
...
function PolygonPreview({ coordinates, annotation = 'SUSPICIOUS', isMultiPolygon = false, onClick }: { coordinates: [number, number][] | [number, number][][], annotation?: string, isMultiPolygon?: boolean, onClick?: () => void }) {
  // Check if we should use globe view for small polygons
  if (shouldUseGlobeView(coordinates, isMultiPolygon)) {
    const multiPolygon = coordinatesToMultiPolygon(coordinates, isMultiPolygon);
    const globeElement = (
      <GlobeView 
  ...
  // Use SVG map preview for larger polygons (better than canvas for geographic data)
  ...
  // Scale coordinates to SVG viewport (100x100)
  const svgSize = 100;
  ...
  const buildPath = () => {
    ...
    polygons.forEach(polyCoords => {
      const scaledCoords = polyCoords.map(scaleCoord);
      path += `M ${scaledCoords.map(([x, y]) => `${x},${y}`).join(' L ')} Z `;
    });
  ...
  const svgElement = (
    <div className={`w-[100px] h-[100px] border border-gray-200 rounded overflow-hidden ${onClick ? 'cursor-pointer hover:opacity-90 transition-all' : ''}`}>
      <svg 
        width="100" 
        height="100" 
        viewBox={`0 0 ${svgSize} ${svgSize}`}
        style={{ backgroundColor: '#CAD2D3' }}
        onClick={onClick}
      >
        {/* Simple land approximation - draw basic shapes for context */}
        {/* This is a simplified approach - in a real app you'd use actual geographic data */}
        <defs>
          <pattern id="landPattern" patternUnits="userSpaceOnUse" width="20" height="20">
            <rect width="20" height="20" fill="#F3F3F1"/>
            <circle cx="5" cy="5" r="1" fill="#E8E8E6" opacity="0.5"/>
            <circle cx="15" cy="15" r="1" fill="#E8E8E6" opacity="0.5"/>
          </pattern>
        </defs>
        
        {/* Create some basic land shapes based on coordinate bounds */}
        {/* This creates a more geographic-looking background */}
        ...
        <g stroke="#B8C1C2" strokeWidth="0.5" opacity="0.4">
  ...
  useEffect(() => {
    ...
    // Set base map background color (water)
    ctx.fillStyle = '#CAD2D3';
    ...
    // Draw simplified land masses in the bounds (optional - for better context)
    // This would require geographic data, so we'll focus on the polygon for now
    
    // Draw a subtle grid for geographic reference
    ctx.strokeStyle = '#B8C1C2';
    ctx.lineWidth = 0.5;
    ctx.globalAlpha = 0.3;
    
    // Draw latitude lines
    const latStep = Math.max(1, Math.round(latRange / 5));
    ...
    const lngStep = Math.max(1, Math.round(lngRange / 5));
    ...
    // Reset alpha for polygon drawing
    ctx.globalAlpha = 1.0;
