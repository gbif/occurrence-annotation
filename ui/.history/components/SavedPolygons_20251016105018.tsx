...
function render3DGlobe(coordinates: [number, number][] | [number, number][][], annotation: string, isMultiPolygon: boolean) {
  ...
  const generateContinentPath = () => {
    ...
    const continents = [
      // North America
      { lat: 45, lng: -100, width: 35, height: 25 },
      // South America
      { lat: -15, lng: -60, width: 18, height: 35 },
      // Europe
      { lat: 55, lng: 20, width: 22, height: 12 },
      // Africa
      { lat: 0, lng: 20, width: 22, height: 45 },
      // Asia
      { lat: 35, lng: 100, width: 55, height: 35 },
      // Australia
      { lat: -25, lng: 135, width: 18, height: 12 }
    ];
    ...
    return continents.map((continent, i) => {
      ...
      // Scale continent size based on distance
      const scale = Math.max(0.3, z / globeRadius);
      const width = continent.width * scale * 0.12;
      const height = continent.height * scale * 0.08;
