...
import { Trash2, Square, Check, X, Edit2, Search, Plus, Minus, ExternalLink, Loader2, MapPin, Calendar, User, Database, Eye, Hand, Repeat } from 'lucide-react';
...
export function MapComponent({
  ...
  const isWithinBounds = (lat: number, lng: number): boolean => {
    // Add a small buffer to prevent drawing exactly on the edge
    const LAT_BUFFER = 0.5; // degrees
    const LNG_BUFFER = 0.5; // degrees
    return lat > (-WEB_MERCATOR_MAX_LAT + LAT_BUFFER) && 
      lat < (WEB_MERCATOR_MAX_LAT - LAT_BUFFER) && 
      lng > (-180 + LNG_BUFFER) && 
      lng < (180 - LNG_BUFFER);
  };
  ...
  return (
    ...
    >
      ...
      {/* Vertex Editing Controls - only show when editing */}
      {editingPolygonId && (
        <div 
          className="absolute top-4 left-4 bg-white rounded-lg shadow-lg p-2 flex flex-col gap-2 z-20"
          onClick={(e) => e.stopPropagation()}
