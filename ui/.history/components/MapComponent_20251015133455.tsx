...
export function MapComponent({
  ...
  const handleMapClick = ({ latLng }: { latLng: [number, number] }) => {
    ...
    // Clamp coordinates to valid map bounds before adding to drawing points
    const clampedCoords = clampCoordinates(lat, lng);
    const [clampedLat, clampedLng] = clampedCoords;
    
    // Log if coordinates were clamped
    if (Math.abs(lat - clampedLat) > 0.001 || Math.abs(lng - clampedLng) > 0.001) {
      console.log('🚫 Coordinates clamped for drawing:', {
        original: `${lat.toFixed(4)}°, ${lng.toFixed(4)}°`,
        clamped: `${clampedLat.toFixed(4)}°, ${clampedLng.toFixed(4)}°`
      });
      toast.info('Drawing point adjusted to stay within map bounds');
    }
    
    setDrawingPoints([...drawingPoints, clampedCoords]);
  };
  ...
  // Clamp coordinates to valid map bounds (for existing systems)
  ...
  // Prevent coordinates from going outside bounds (returns original if invalid)
  const preventOutOfBounds = (newLat: number, newLng: number, originalLat: number, originalLng: number): [number, number] => {
    if (isWithinBounds(newLat, newLng)) {
      return [newLat, newLng];
    }
    // Return original coordinates if new ones would be out of bounds
  ...
  const handleOverlayMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (isDraggingShape && dragStart) {
      ...
      const y = e.clientY - rect.top;
      const latLng = pixelToCurrentLatLng(x, y);
      const clampedLatLng = clampCoordinates(latLng[0], latLng[1]);
      
      setDragCurrent(clampedLatLng);
      return;
    }
    ...
    if (draggingVertex) {
      ...
      const clampedLatLng = clampCoordinates(newLatLng[0], newLatLng[1]);
      
      if (draggingVertex.polygonId === 'current' && currentPolygon) {
        const newCoordinates = [...currentPolygon];
        newCoordinates[draggingVertex.index] = clampedLatLng;
        onPolygonChange(newCoordinates);
          ...
          if (polygon.isMultiPolygon) {
            ...
            for (let partIndex = 0; partIndex < coords.length; partIndex++) {
              if (totalIndex + coords[partIndex].length > draggingVertex.index) {
                ...
                newCoords[partIndex][vertexInPart] = clampedLatLng;
            ...
            const coords = polygon.coordinates as [number, number][];
            const newCoords = [...coords];
            newCoords[draggingVertex.index] = clampedLatLng;
            onUpdatePolygon(draggingVertex.polygonId, newCoords);
          }
        }
      }
    ...
    if (draggingPolygon && onUpdatePolygon) {
      ...
      // Apply delta to all coordinates with clamping
