        ...
        >
          ...
          <g>
            ...
            */}
            ...
            {/* Saved Polygon Edge Midpoints (for adding vertices) - only show when editing and not dragging or zooming */}
            {!isZooming && savedPolygons.map((polygon) => {
              const isEditing = editingPolygonId === polygon.id;
              if (!isEditing || draggingVertex) return null;
            ...
            {/* Saved Polygon Vertices - only show when editing and not dragging or zooming */}
            {!isZooming && savedPolygons.map((polygon) => {
              const isEditing = editingPolygonId === polygon.id;
              
              // Only render vertices when editing this polygon and not dragging any vertex
              if (!isEditing || draggingVertex) return null;
              ...
              const vertexColor = annotationColors[annotation.toUpperCase()] || '#3b82f6';
                  ...
                  return (
                    <g key={`saved-${polygon.id}-${partIndex}-${index}`}>
                      ...
                      <circle
                        ...
                        onContextMenu={(e) => handleVertexRightClick(e, polygon.id, encodedIndex)}
                        style={{ 
                          cursor: 'move'
                        }}
                      >
                        <title>Drag to move • Right-click to delete (Part {partIndex + 1})</title>
                      </circle>
                    </g>
                  );
                })
