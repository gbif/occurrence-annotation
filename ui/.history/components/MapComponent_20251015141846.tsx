...
export function MapComponent({
  ...
  const handleSavedPolygonEdgeClick = (e: React.MouseEvent, polygonId: string, partIndex: number, edgeStartIndex: number) => {
    e.stopPropagation();
    e.preventDefault();
    ...
    if (polygon.isMultiPolygon) {
      const coords = polygon.coordinates as [number, number][][];
      const part = coords[partIndex];
      if (!part) return;
      
      // Calculate midpoint
      const point1 = part[edgeStartIndex];
      const point2 = part[(edgeStartIndex + 1) % part.length];
      const midLat = (point1[0] + point2[0]) / 2;
      const midLng = (point1[1] + point2[1]) / 2;
      ...
      newCoords[partIndex] = [
        ...part.slice(0, edgeStartIndex + 1),
        [midLat, midLng] as [number, number],
        ...part.slice(edgeStartIndex + 1),
      ];
      
      onUpdatePolygon(polygonId, newCoords);
    } else {
      const coords = polygon.coordinates as [number, number][];
      
      // Calculate midpoint
      const point1 = coords[edgeStartIndex];
      const point2 = coords[(edgeStartIndex + 1) % coords.length];
      const midLat = (point1[0] + point2[0]) / 2;
      const midLng = (point1[1] + point2[1]) / 2;
      
      // Insert new vertex
      const newCoords = [
        ...coords.slice(0, edgeStartIndex + 1),
        [midLat, midLng] as [number, number],
        ...coords.slice(edgeStartIndex + 1),
      ];
                ...
                <svg width="4000" height="4000" style={{ overflow: 'visible' }}>
                  ...
                  {isEditing && !isZooming && !draggingPolygon && !draggingVertex && polygonParts.map((polyCoords, partIndex) => {
                    ...
                    return polyCoords.map((coord, vertexIndex) => {
                      ...
                      return (
                        <g key={`edge-${partIndex}-${vertexIndex}`}>
                          ...
                          <circle
                            ...
                            onClick={(e) => {
                              handleSavedPolygonEdgeClick(e, polygonData.id, partIndex, vertexIndex);
                            }}
                          />
                          {/* Plus symbol */}
                          <g 
                    ...
                    return (
                      <>
                        ...
                        {!draggingPolygon && !draggingVertex && currentPolygon.map((coord, vertexIndex) => {
                          ...
                          return (
                            <g key={`current-edge-${vertexIndex}`}>
                              {/* Background circle for the + button */}
                              <circle
                                cx={offsetX}
                                cy={offsetY}
