...
function SaveToGBIFDialog({ polygon, onSuccess, annotation }: SaveToGBIFDialogProps) {
  ...
  const postPendingComments = async (ruleId: number, gbifAuth: string) => {
    try {
      ...
      const successfulComments = pendingComments.filter((c: any) => c.status === 'saved').length;
  ...
  const handleSave = async () => {
    ...
    if (!gbifAuth) {
      toast.error('Please login to GBIF first');
      return;
    }

    if (!polygon.species) {
      toast.error('Cannot save: polygon has no associated species');
      return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button
          size="icon"
          variant="outline"
          className="h-9 w-9 border-green-300 text-green-700 hover:bg-green-50 hover:border-green-400"
          disabled={!polygon.species}
          title={!polygon.species ? "Select a species first" : "Save to GBIF"}
        >
...
function PolygonCard({ 
  ...
  const handleAddComment = async () => {
    ...
    let gbifAuthStr;
    try {
      gbifAuthStr = localStorage.getItem('gbifAuth');
    } catch (err) {
      console.error('Error accessing localStorage:', err);
      toast.error('Error accessing browser storage');
      return;
    }

    if (!gbifAuthStr) {
      console.log('No GBIF auth found');
    ...
    try {
      console.log('Parsing GBIF auth...');
      let gbifAuth;
      try {
        gbifAuth = JSON.parse(gbifAuthStr);
      } catch (parseErr) {
        console.error('Error parsing GBIF auth:', parseErr);
        throw new Error('Invalid authentication data. Please login again.');
      }

      if (!gbifAuth.userName) {
        throw new Error('Authentication data is missing username');
      }

      console.log('GBIF auth parsed successfully, user:', gbifAuth.userName);
      ...
      <Dialog open={commentDialogOpen} onOpenChange={setCommentDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Add a comment that will be automatically posted to GBIF when this polygon is saved as an annotation rule.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-amber-600">
                💡 This comment will be stored locally until you save the polygon as a rule to GBIF
              </p>
            </div>
          </div>
          <DialogFooter>
