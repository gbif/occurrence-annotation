...
export function AnnotationRules({ 
  ...
  const handleEditRule = async (ruleId: number) => {
    ...
    if (!isLoggedIn) {
      toast.error('You must be logged in to GBIF to edit rules');
      return;
    }
    ...
    try {
      ...
      if (!authHeader) {
        toast.error('No valid authentication found. Please log in to GBIF again.');
        return;
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          // For any other error, get more details
          let errorMessage = `Failed to delete rule from server (${response.status})`;
          try {
        ...
        toast.error('Network error while deleting rule from server');
        return;
      }
