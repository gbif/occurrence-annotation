...
export function InvestigateArea({ 
  ...
  const [searchRadius] = useState(10000); // Default 10km radius
  ...
  const investigateArea = useCallback(async (lat: number, lng: number) => {
    ...
    try {
      ...
      // Simple API call to test basic functionality first
  ...
  return (
    <>
      ...
      <div className="flex gap-2">
        ...
        {/* Debug button to test investigate function directly */}
        {isInvestigateMode && selectedSpecies && (
          <Button
            variant="outline"
            size="sm"
            onClick={() => {
              console.log('🔍 InvestigateArea: Manual test trigger');
              investigateArea(0, 0); // Test with coordinates 0,0
            }}
            title="Test investigate function"
          >
            Test
          </Button>
        )}
