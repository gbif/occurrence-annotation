...
import { Trash2, Square, Check, X, Edit2, Search, Plus, Minus, ExternalLink, Loader2, MapPin, Calendar, User, Database, Eye, Hand, Repeat, GitBranch, Scissors, Layers } from 'lucide-react';
...
export function MapComponent({
  ...
  // Latitude band state
  const [latBandCenter, setLatBandCenter] = useState(0); // Start at equator
  const [latBandWidth, setLatBandWidth] = useState(6); // Default 6 degrees thick (3 degrees on each side)
  ...
  const createLatitudeBand = () => {
    ...
    const halfWidth = latBandWidth / 2;
    const northLat = Math.min(85, latBandCenter + halfWidth); // Clamp to avoid pole issues
    const southLat = Math.max(-85, latBandCenter - halfWidth);
  ...
  const adjustLatitudeBand = (widthDelta: number, centerDelta: number = 0) => {
    const newWidth = Math.max(1, Math.min(170, latBandWidth + widthDelta)); // Keep between 1 and 170 degrees
    const newCenter = Math.max(-85, Math.min(85, latBandCenter + centerDelta)); // Keep within map bounds
    
    setLatBandWidth(newWidth);
    setLatBandCenter(newCenter);
    ...
    const northLat = Math.min(85, newCenter + halfWidth);
    const southLat = Math.max(-85, newCenter - halfWidth);
      ...
      >
        ...
        {!isDrawing ? (
          <>
            ...
            {/* Latitude Band Controls - show when band is active */}
            {showLatBandControls && currentPolygon && (
              <div 
                className="flex flex-col gap-1 bg-white rounded border shadow-sm px-1 py-1 relative z-30"
                onClick={(e) => e.stopPropagation()}
                onMouseDown={(e) => e.stopPropagation()}
              >
                <div className="text-xs font-medium text-center py-1 text-gray-600">Latitude Band</div>
                ...
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      adjustLatitudeBand(1, 0);
                    }}
                    title="Expand band"
                  >
                    <Plus className="w-3 h-3" />
                  </Button>
                  <span className="text-xs font-medium text-center px-1 min-w-[3rem]">
                    {latBandWidth.toFixed(1)}°
                  </span>
                  <Button
                    size="icon"
                    variant="ghost"
