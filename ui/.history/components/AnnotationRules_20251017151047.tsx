...
export function AnnotationRules({ 
  ...
  const handleEditRule = async (ruleId: number) => {
    ...
    try {
      ...
      // Then try to delete the old rule from GBIF (copy exact logic from working delete function)
      const gbifAuth = localStorage.getItem('gbifAuth');
      if (!gbifAuth) {
        toast.error('Please login to GBIF to delete annotation rules');
        return;
      ...
      try {
        console.log(`Attempting to delete rule ${ruleId} for editing...`);
        const response = await fetch(
          `https://api.gbif.org/v1/occurrence/experimental/annotation/rule/${ruleId}`,
          {
            method: 'DELETE',
        ...
        if (!response.ok) {
          if (response.status === 401) {
            ...
            toast.error(`Failed to delete rule (${response.status})`);
            return;
          }
        } else {
          console.log(`Successfully deleted rule ${ruleId} for editing`);
