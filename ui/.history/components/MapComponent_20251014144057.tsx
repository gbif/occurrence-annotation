...
export function MapComponent({
  ...
  const handleOverlayMouseMove = (e: React.MouseEvent) => {
    ...
    if (draggingPolygon && onUpdatePolygon) {
      ...
      const deltaX = currentPos[0] - draggingPolygon.startPos[0];
      const deltaY = currentPos[1] - draggingPolygon.startPos[1];
    ...
    // Handle vertex dragging (legacy - now removed)
    if (draggingVertex) {
      const rect = mapContainerRef.current?.getBoundingClientRect();
      if (!rect) return;
          ...
          return (
            <Overlay key={`overlay-${polygonData.id}`} anchor={[anchorLat, anchorLng]} offset={[0, 0]}>
              <div 
                style={{ 
                  pointerEvents: 'auto',
                  cursor: draggingPolygon?.id === polygonData.id ? 'grabbing' : 'grab'
                }}
                onMouseDown={(e) => {
                  e.stopPropagation();
                  const rect = mapContainerRef.current?.getBoundingClientRect();
