...
import { Trash2, Square, Check, X, Edit2, Search } from 'lucide-react';
...
export function MapComponent({
  ...
  onSaveAndEdit,
  onAutoSave,
  isInvestigateMode = false,
  investigateRadius = 5000,
}: MapComponentProps) {
  console.log('🗺️ MapComponent: Rendered with isInvestigateMode =', isInvestigateMode);
  ...
  const handleMapClick = ({ latLng }: { latLng: [number, number] }) => {
    ...
    console.log('🗺️ MapComponent: handleMapClick called with:', { lat, lng, isInvestigateMode });
    ...
    if (isInvestigateMode) {
      ...
      if ((window as any).__investigateArea) {
        console.log('🗺️ MapComponent: Calling investigate function');
        (window as any).__investigateArea(lat, lng);
      } else {
        console.log('🗺️ MapComponent: No investigate function available');
      }
      return;
    }
  ...
  return (
    <div 
      ...
      onClick={(e) => {
        console.log('🗺️ Container div clicked! isInvestigateMode:', isInvestigateMode, 'event:', e);
        if (isInvestigateMode) {
      ...
      {mapSize.width > 0 && mapSize.height > 0 && (
        ...
        >
          ...
          {isInvestigateMode && mousePosition && (() => {
            ...
            // At zoom level, 1 degree of latitude ≈ 111,000 meters
            const metersPerPixelAtEquator = 40075016.686 / (256 * Math.pow(2, zoom)); // Earth circumference / (tile size * tiles at zoom)
            ...
            // Calculate radius in pixels, with a minimum of 12 and maximum of 100 for visibility
            const pixelRadius = Math.max(12, Math.min(100, investigateRadius / metersPerPixel));
            
            // Make crosshair size proportional to circle size
            const crosshairSize = Math.max(6, Math.min(16, pixelRadius * 0.4));
