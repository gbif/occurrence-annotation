    ...
    <div className={`border border-gray-200 rounded overflow-hidden ${className}`}
      ...
      >
        ...
        {polygonCoordinates.map((ring, index) => {
          ...
          if (isInverted) {
            // For inverted polygons, create a world-covering shape with a hole
            const worldPath = `M 0 0 L ${width} 0 L ${width} ${height} L 0 ${height} Z`;
            const combinedPath = `${worldPath} ${pathString}`;
            ...
            return (
              <path
                ...
                strokeWidth={0.8}
                fillRule="evenodd" // This makes the inner path a hole
                style={{ outline: "none" }}
              />
            );
          } else {
