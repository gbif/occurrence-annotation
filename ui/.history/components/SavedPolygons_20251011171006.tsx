...
import { Trash2, Repeat, Edit2, Upload, FileText, Loader2, Plus, Check } from 'lucide-react';
    ...
    <Card className={`p-4 ${isEditing ? 'ring-2 ring-blue-500 bg-blue-50' : ''} ${isAddingTo ? 'ring-2 ring-green-500 bg-green-50' : ''}`}>
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-gray-200">
          {onAddToMultiPolygon && (
            <Button
              size={isAddingTo ? "sm" : "icon"}
              variant={isAddingTo ? "default" : "outline"}
              onClick={() => onAddToMultiPolygon(isAddingTo ? null : polygon.id)}
              className={`${isAddingTo ? 'h-9 gap-1 bg-green-600 hover:bg-green-700 text-white' : 'h-9 w-9'}`}
            >
              {isAddingTo ? (
                <>
