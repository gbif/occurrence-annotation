        ...
        taxonKeys.forEach(({ key, level }) => {
          ...
          switch (level) {
            case 'selected':
              ...
              break;
            case 'genus':
              // For genus, we'll need to fetch the scientific name or use a simplified approach
              scientificName = selectedSpecies.scientificName.split(' ')[0]; // First word is usually genus
              break;
            case 'family':
            case 'order':
            case 'class':
            case 'phylum':
            case 'kingdom':
              // For higher taxonomic levels, we can use the level name as placeholder
              // In a full implementation, you'd fetch the actual scientific names from GBIF API
              scientificName = `${level.charAt(0).toUpperCase() + level.slice(1)} of ${selectedSpecies.scientificName.split(' ')[0]}`;
              break;
          }
