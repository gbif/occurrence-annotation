...
function PolygonCard({ 
  ...
  // Auto-invert hack: If polygon has the inverted property set to true but isn't currently inverted, trigger invert
  useEffect(() => {
    if (polygon.inverted === true && !polygon.inverted) {
      console.log('🔄 Auto-triggering invert for polygon that should be inverted:', polygon.id);
      // Small delay to ensure component is fully mounted
      setTimeout(() => {
        onToggleInvert(polygon.id);
      }, 100);
    }
  }, [polygon.id, polygon.inverted, onToggleInvert]);
