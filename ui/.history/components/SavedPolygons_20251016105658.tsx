      ...
      <div className="space-y-3">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-shrink-0 flex flex-col items-center gap-1">
            ...
            {render3DGlobe(polygon.coordinates, annotation, !!polygon.isMultiPolygon)}
...
function isPolygonLocalScale(coordinates: [number, number][] | [number, number][][], isMultiPolygon: boolean): boolean {
  const allCoords = isMultiPolygon 
    ? (coordinates as [number, number][][]).flat()
    : (coordinates as [number, number][]);
...
// Get polygon fill color based on annotation
function getPolygonFillColor(annotation: string): string {
  const colors: { [key: string]: string } = {
    SUSPICIOUS: 'rgba(239, 68, 68, 0.2)',
    NATIVE: 'rgba(16, 185, 129, 0.2)',
    MANAGED: 'rgba(59, 130, 246, 0.2)',
    FORMER: 'rgba(168, 85, 247, 0.2)',
    VAGRANT: 'rgba(249, 115, 22, 0.2)',
  };
...
function render3DGlobe(coordinates: [number, number][] | [number, number][][], annotation: string, isMultiPolygon: boolean) {
  if (!isPolygonLocalScale(coordinates, isMultiPolygon)) return null;
  ...
  const globeCenter = 25; // Center of 50x50 SVG
  ...
  const generateWireframe = () => {
    ...
    for (let lat = -60; lat <= 60; lat += 30) {
      ...
      const radius = globeRadius * Math.cos(latRad);
      const yPos = globeCenter - globeRadius * Math.sin(latRad) * Math.cos(rotX);
      ...
      if (radius > 0) {
        lines.push(
          <ellipse
            ...
            cy={yPos}
            rx={radius}
            ry={radius * Math.sin(Math.PI/3)} // Perspective distortion
            fill="none"
            stroke="#0284c7"
            strokeWidth="0.4"
            opacity="0.4"
    ...
    for (let lng = 0; lng < 180; lng += 30) {
      ...
      const visible = Math.cos(lngRad) > 0; // Only show front-facing meridians
      ...
      if (visible) {
        lines.push(
          <ellipse
            ...
            cx={globeCenter}
            cy={globeCenter}
            rx={globeRadius * Math.abs(Math.sin(lngRad))}
            ry={globeRadius}
            fill="none"
            stroke="#0284c7"
            strokeWidth="0.4"
  ...
  // Generate continent outlines with better mapping
  const generateContinentPath = () => {
    // More detailed continent shapes with better coordinates
    const continents = [
      // North America
      { coords: [[70, -160], [60, -140], [50, -125], [25, -80], [30, -95], [45, -120], [60, -165]] },
      // South America  
      { coords: [[12, -82], [5, -35], [-25, -40], [-55, -65], [-30, -85], [0, -78]] },
      // Europe
      { coords: [[71, -10], [71, 30], [60, 40], [35, 40], [35, 0], [50, -5]] },
      // Africa
      { coords: [[37, -18], [37, 52], [5, 45], [-35, 20], [-35, 10], [15, -5]] },
      // Asia
      { coords: [[75, 40], [75, 180], [50, 160], [10, 140], [20, 60], [40, 35]] },
      // Australia
      { coords: [[-10, 110], [-40, 155], [-40, 110]] }
    ];
    
    return continents.map((continent, continentIndex) => {
      const points = continent.coords.map(([lat, lng]) => {
        // Apply orthographic projection with rotation
        const latRad = lat * Math.PI / 180;
        const lngRad = lng * Math.PI / 180;
        
        const cosLat = Math.cos(latRad);
        const sinLat = Math.sin(latRad);
        const cosLng = Math.cos(lngRad + rotY);
        const sinLng = Math.sin(lngRad + rotY);
        ...
        return { x: globeCenter + x, y: globeCenter + y, z };
      });
      
      // Only render if most points are facing forward
      const visiblePoints = points.filter(p => p.z > 0);
      if (visiblePoints.length < points.length * 0.3) return null;
      
      const pathData = points.map((point, i) => {
        if (point.z < 0) return ''; // Skip points behind globe
