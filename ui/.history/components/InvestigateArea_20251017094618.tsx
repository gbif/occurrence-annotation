...
export function InvestigateArea({ 
  ...
  const [searchRadius, setSearchRadius] = useState(100000); // Default 100km radius
  ...
  // Function to fetch occurrences with pagination
  const fetchOccurrences = async (lat: number, lng: number, currentOffset: number, isLoadMore: boolean) => {
    try {
      ...
      const newHasMore = (currentOffset + 20) < (data.count || 0);
      ...
      if (isLoadMore) {
        setOccurrences(prev => [...prev, ...enrichedOccurrences]);
      } else {
        setOccurrences(enrichedOccurrences);
      }
      
      if (enrichedOccurrences.length === 0 && !isLoadMore) {
        toast.info(`No occurrences found for ${selectedSpecies?.scientificName} within ${searchRadius/1000}km of this location`);
      } else if (!isLoadMore) {
        toast.success(`Found ${data.count || 0} occurrence(s) for ${selectedSpecies?.scientificName} (showing first 20)`);
      }
  ...
  const loadMoreResults = useCallback(async () => {
    if (!investigationPoint || !hasMore || isLoadingMore) return;
    
    setIsLoadingMore(true);
    await fetchOccurrences(investigationPoint.lat, investigationPoint.lng, offset, true);
  }, [investigationPoint, hasMore, isLoadingMore, offset, searchRadius, selectedSpecies]);
          ...
          <>
            <div className="flex items-center gap-1 bg-white rounded-lg shadow-sm border px-2 py-1">
              ...
              <Button
                ...
                className="h-6 w-6"
                onClick={() => {
                  const newRadius = Math.min(100000, searchRadius + 1000);
                  setSearchRadius(newRadius);
                }}
                disabled={searchRadius >= 100000}
                ...
                <div className="flex justify-center pt-4">
                  ...
                  >
                    {isLoadingMore ? (
                      ...
                      <>
                        Load 20 more ({totalCount - occurrences.length} remaining)
                      </>
                    )}
                  </Button>
                </div>
