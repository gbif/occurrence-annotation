...
import { Trash2, Square, Check, X, Edit2, MessageSquare, Loader2 } from 'lucide-react';
...
// GBIF base map tile provider
const gbifTileProvider = (x: number, y: number, z: number) => {
  return `https://tile.gbif.org/3857/omt/${z}/${x}/${y}@1x.png`;
};
...
export function MapComponent({
  ...
  useEffect(() => {
    ...
    const [centerX, centerY] = latLngToTile(center[0], center[1], tileZoom);
    ...
    for (let dx = -Math.floor(tilesX / 2); dx <= Math.ceil(tilesX / 2); dx++) {
      for (let dy = -Math.floor(tilesY / 2); dy <= Math.ceil(tilesY / 2); dy++) {
        ...
        if (x >= 0 && x < maxTile && y >= 0 && y < maxTile) {
          ...
          const anchor = tileToLatLng(x, y, tileZoom);
  ...
  const handleAddComment = async (polygonId: string) => {
    if (!newComment.trim()) {
    ...
    try {
      ...
      const credentials = btoa(`${gbifAuth.userName}:${gbifAuth.password}`);

      setSubmittingComment(true);

      // For now, we'll store comments locally since these are user-created polygons
      // In a real implementation, you might want to create an annotation rule first
      // or have a separate API endpoint for polygon comments
      
      // Store comment locally with timestamp
      const comment = {
        polygonId,
        comment: newComment.trim(),
        timestamp: new Date().toISOString(),
        user: gbifAuth.userName
      };
  ...
  const getCommentCount = (polygonId: string): number => {
    try {
      const existingComments = JSON.parse(localStorage.getItem('polygonComments') || '[]');
      return existingComments.filter((comment: any) => comment.polygonId === polygonId).length;
    } catch {
      return 0;
    }
  };
  ...
  return (
    ...
    >
      ...
      {mapSize.width > 0 && mapSize.height > 0 && (
        ...
        >
          ...
          >
            ...
            {/* Comment count indicators for saved polygons */}
            {savedPolygons.map((polygonData) => {
              const commentCount = getCommentCount(polygonData.id);
              if (commentCount === 0) return null;
              
              // Get centroid of first polygon part for comment indicator position
              const polygonParts: [number, number][][] = polygonData.isMultiPolygon 
                ? (polygonData.coordinates as [number, number][][])
                : [polygonData.coordinates as [number, number][]];
              
              if (polygonParts.length === 0 || polygonParts[0].length === 0) return null;
              
              // Calculate centroid of the first polygon part
              const coords = polygonParts[0];
              ...
              const [centroidX, centroidY] = latLngToStablePixel(centroidLat, centroidLng);
              
              return (
                <g key={`comment-indicator-${polygonData.id}`}>
                  {/* Comment count badge */}
                  <circle
                    cx={centroidX}
                    cy={centroidY}
            ...
            {(() => {
              ...
              console.log('DEBUG: Current Map Bounds', {
                ...
                zoom: zoom,
                bounds: mapBounds,
                webMercatorLimit: `±${WEB_MERCATOR_MAX_LAT}°`,
      ...
      {/* Editing Controls - show when editing a polygon */}
      {editingPolygonId && (
        <div className="absolute top-4 left-4 bg-white rounded-lg shadow-lg p-2 flex flex-col gap-2 z-10">
          <Button 
            onClick={() => setCommentDialogOpen(editingPolygonId)} 
            size="icon" 
            variant="outline"
            title={`Add Comment${getCommentCount(editingPolygonId) > 0 ? ` (${getCommentCount(editingPolygonId)} existing)` : ''}`}
            className="border-blue-300 text-blue-600 hover:bg-blue-50"
          >
      ...
      {/* Comment Dialog */}
      <Dialog open={commentDialogOpen !== null} onOpenChange={(open) => !open && setCommentDialogOpen(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Add Comment to Polygon</DialogTitle>
            <DialogDescription>
              Add a comment or note about this polygon for future reference.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button 
              onClick={() => commentDialogOpen && handleAddComment(commentDialogOpen)}
