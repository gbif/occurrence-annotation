...
function PolygonPreview({ coordinates, annotation = 'SUSPICIOUS', isMultiPolygon = false, onClick }: { coordinates: [number, number][] | [number, number][][], annotation?: string, isMultiPolygon?: boolean, onClick?: () => void }) {
  ...
  useEffect(() => {
    ...
    // Draw world map background (only for global/large views)
    const drawWorldMap = () => {
      const isGlobalView = (maxLng - minLng) > 180 || (maxLat - minLat) > 90;
      if (!isGlobalView) return; // Only show for global/large views
      
      ctx.save();
      ctx.globalAlpha = 0.15;
      ctx.fillStyle = '#6b7280';
      
      // Scale world coordinates to canvas view bounds
      ...
      // Draw simplified continents (only if they intersect with view)
      const continents = [
        // North America
        { coords: [[60, -140], [70, -100], [50, -60], [25, -80], [20, -100], [45, -160]] },
        // South America  
        { coords: [[10, -80], [5, -35], [-20, -40], [-55, -65], [-20, -85]] },
        // Europe
        { coords: [[70, -10], [60, 40], [40, 30], [50, 0]] },
        // Africa
        { coords: [[35, 0], [35, 50], [0, 45], [-35, 20], [-35, 10], [15, 0]] },
        // Asia
        { coords: [[70, 40], [70, 180], [20, 140], [10, 60]] },
        // Australia
        { coords: [[-10, 110], [-40, 155], [-40, 110]] }
      ];
      
      continents.forEach(continent => {
        // Check if continent intersects with current view bounds
        const continentLats = continent.coords.map(([lat]) => lat);
        const continentLngs = continent.coords.map(([, lng]) => lng);
        const contMinLat = Math.min(...continentLats);
        const contMaxLat = Math.max(...continentLats);
        const contMinLng = Math.min(...continentLngs);
        const contMaxLng = Math.max(...continentLngs);
        
        // Check for intersection
        const intersects = !(contMaxLat < minLat || contMinLat > maxLat || 
                           contMaxLng < minLng || contMinLng > maxLng);
        
        if (intersects) {
          ctx.beginPath();
          continent.coords.forEach(([lat, lng], i) => {
            const [x, y] = worldToCanvas(lat, lng);
