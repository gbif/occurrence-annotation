...
export function MapComponent({
  ...
  useEffect(() => {
    ...
    console.log('📍 GBIF TILES LOADED (SVG):', {
      tileCount: newTiles.length,
      species: selectedSpecies?.name || 'Unknown',
      zoom: zoom,
      renderMethod: 'SVG (no coordinate interference)',
      status: '🔍 Check if coordinate shift is FIXED'
    });
  ...
  return (
    ...
    >
      <Map
        ...
        attribution={false}
      >
        {/* GBIF Occurrence Tiles as Direct SVG Images */}
        {/* Moved from Overlay to SVG to prevent coordinate system interference */}
      ...
      {mapSize.width > 0 && mapSize.height > 0 && (
        ...
        >
          ...
          >
            {/* GBIF Occurrence Tiles - rendered in SVG to avoid coordinate interference */}
            {!isZooming && gbifTiles.map((tile) => {
              ...
              // Calculate tile position using our stable coordinate system
              const [tileX, tileY] = latLngToStablePixel(nwLat, nwLng);
              
              return (
                <image
                  key={`gbif-svg-${tile.z}-${tile.x}-${tile.y}`}
                  href={tile.url}
                  x={tileX}
                  y={tileY}
                  width={displaySize}
