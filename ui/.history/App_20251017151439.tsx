  ...
  const handleEditRule = useCallback((multiPolygon: MultiPolygon) => {
    try {
      ...
      if (multiPolygon.polygons.length === 1) {
        ...
        // For inverted single polygons, if there are holes, use the first hole as the main editing polygon
        if (isInverted && polygon.holes && polygon.holes.length > 0) {
          coordinates = polygon.holes[0]; // Use the first hole as the main polygon to edit
          console.log('🔄 Using hole as main polygon for inverted single polygon editing');
        }
      } else {
        // Multiple polygons - check if this is an inverted multipolygon
        const hasInvertedPolygon = multiPolygon.polygons.some(polygon => isInvertedPolygon(polygon));
        
        if (hasInvertedPolygon) {
          // For inverted multipolygons, find the polygon with holes and use the first hole
          const invertedPolygon = multiPolygon.polygons.find(polygon => isInvertedPolygon(polygon));
          if (invertedPolygon && invertedPolygon.holes && invertedPolygon.holes.length > 0) {
            // Use the first hole as the main editing polygon
            coordinates = invertedPolygon.holes[0];
            isInverted = true;
            isMultiPolygon = false; // Edit as single polygon (the hole)
            console.log('🔄 Converting inverted multipolygon to single polygon editing using first hole');
          } else {
            // Fallback: treat as regular multipolygon
            coordinates = multiPolygon.polygons.map(polygon => polygon.outer);
            isMultiPolygon = true;
            isInverted = false;
          }
        } else {
          // Regular multipolygon
          coordinates = multiPolygon.polygons.map(polygon => polygon.outer);
          isMultiPolygon = true;
          isInverted = false;
        }
      ...
      // For proper editing, set as current polygon for single polygons (including converted inverted ones)
      if (!isMultiPolygon) {
        setCurrentPolygon(coordinates as [number, number][]);
        console.log('🎯 Set current polygon for editing:', {
          coordinatesCount: (coordinates as [number, number][]).length,
          isInverted
        });
      } else {
        // For true multipolygons, clear current polygon to avoid conflicts
        setCurrentPolygon(null);
        console.log('🎯 Cleared current polygon for multipolygon editing');
      }
