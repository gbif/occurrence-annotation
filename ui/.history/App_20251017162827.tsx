...
export default function App() {
  ...
  const handleToggleInvert = useCallback((id: string) => {
    setSavedPolygons(prev => prev.map(p => {
      if (p.id === id) {
        const newInverted = !p.inverted;
        // If this is the currently editing polygon, also update global inversion state
        if (id === editingPolygonId) {
          setIsInverted(newInverted);
          console.log('🔄 Toggled inversion for editing polygon:', {
            id,
            newInverted,
            isMultiPolygon: p.isMultiPolygon
          });
        }
        return { ...p, inverted: newInverted };
      }
      return p;
    }));
  }, [editingPolygonId]);
  ...
  const handleEditRule = useCallback((multiPolygon: MultiPolygon) => {
    try {
      ...
      const isInvertedPolygon = (polygon: any): boolean => {
        ...
        console.log('🔍 DEBUG: Checking if polygon is inverted:', {
          bounds: { minLat, maxLat, minLng, maxLng },
          latSpan: maxLat - minLat,
          lngSpan: maxLng - minLng,
          coversWorld,
          hasHoles,
          holeCount: polygon.holes ? polygon.holes.length : 0,
          isInverted: coversWorld && hasHoles
        });
      ...
      if (multiPolygon.polygons.length === 1) {
        ...
        // Multiple polygons - check if any are inverted first
        const hasInvertedPolygon = multiPolygon.polygons.some(polygon => isInvertedPolygon(polygon));
        
        if (hasInvertedPolygon) {
          // For inverted multipolygons, we need to include the holes as separate polygons
          const allPolygons: [number, number][][] = [];
          
          console.log('🔍 DEBUG: Processing inverted multipolygon:', {
            totalPolygons: multiPolygon.polygons.length,
            polygonsData: multiPolygon.polygons.map((p, i) => ({
              index: i,
              outerLength: p.outer.length,
              holesCount: p.holes ? p.holes.length : 0,
              isInverted: isInvertedPolygon(p),
              outerBounds: {
                lats: [Math.min(...p.outer.map(c => c[0])), Math.max(...p.outer.map(c => c[0]))],
                lngs: [Math.min(...p.outer.map(c => c[1])), Math.max(...p.outer.map(c => c[1]))]
              }
            }))
          });
          
          multiPolygon.polygons.forEach((polygon, polygonIndex) => {
            if (isInvertedPolygon(polygon)) {
              console.log(`🔍 DEBUG: Polygon ${polygonIndex} is inverted, holes:`, polygon.holes?.length || 0);
              // For inverted polygons, add all holes as separate polygons
              if (polygon.holes && polygon.holes.length > 0) {
                polygon.holes.forEach((hole, holeIndex) => {
                  console.log(`🔍 DEBUG: Adding hole ${holeIndex} with ${hole.length} points`);
                  allPolygons.push(hole);
                });
              } else {
                console.log(`⚠️ DEBUG: Inverted polygon ${polygonIndex} has no holes!`);
              }
            } else {
              console.log(`🔍 DEBUG: Polygon ${polygonIndex} is NOT inverted, adding outer ring`);
              // For regular polygons, add the outer ring
              allPolygons.push(polygon.outer);
            }
          });
          
          coordinates = allPolygons;
          isMultiPolygon = true;
          isInverted = true; // Keep as inverted for editing purposes
          
          console.log('🔄 Final result - extracted coordinates:', {
            originalPolygonCount: multiPolygon.polygons.length,
            resultingPolygonCount: allPolygons.length,
            coordinatesPreview: allPolygons.map((coords, i) => ({
              index: i,
              pointCount: coords.length,
              bounds: coords.length > 0 ? {
                lats: [Math.min(...coords.map(c => c[0])), Math.max(...coords.map(c => c[0]))],
                lngs: [Math.min(...coords.map(c => c[1])), Math.max(...coords.map(c => c[1]))]
              } : null
            }))
          });
        } else {
          // Regular multipolygon - use outer rings
      ...
      // Never set currentPolygon when editing rules - everything should happen in Active Rules cards
      setCurrentPolygon(null);
      console.log('🎯 Rule loaded for editing in Active Rules card (no currentPolygon created)');
      ...
      console.log('🔄 Edit rule loaded:', {
        id: newPolygonData.id,
        isMultiPolygon,
        isInverted,
        coordinatesType: Array.isArray(coordinates[0]) ? 'multipolygon' : 'single polygon'
      });
