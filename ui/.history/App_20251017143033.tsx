  ...
  const handleEditRule = useCallback((multiPolygon: MultiPolygon) => {
    try {
      ...
      const newPolygonData: PolygonData = {
        ...
        species: selectedSpecies,
        timestamp: new Date().toISOString(),
        inverted: false, // Will need to detect if this should be true
        annotation: currentAnnotation,
        isMultiPolygon
      };

      // Add to saved polygons and set as current polygon
      setSavedPolygons(prev => [...prev, newPolygonData]);
      
      // Set as current polygon for editing
      if (!isMultiPolygon) {
        setCurrentPolygon(coordinates as [number, number][]);
      }
      
      toast.success('Rule loaded for editing. Polygon is now active on the map.');
    } catch (error) {
