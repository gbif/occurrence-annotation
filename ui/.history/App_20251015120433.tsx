...
export default function App() {
  ...
  const handleRuleSavedToGBIF = useCallback(() => {
    // Clear the current active polygon
    setCurrentPolygon(null);
    setIsInverted(false);
    
    // Show success message that disappears after 3 seconds
    const toastId = toast.success('🎉 Annotation rule saved to GBIF!', {
      description: 'Your polygon has been successfully saved as an annotation rule.',
      duration: 3000,
    });
    
    console.log('📤 Rule saved to GBIF - current polygon cleared');
  }, []);
